...
class SimpleImage {
  ...
  public function textBox($text, $options) {
    ...
    // Justify
      ...
      foreach ($lines as $keyLine => $line) {
        ...
        // Separate words
        $nrgrds = preg_split("/\s+/", $line);
        // Include spaces with the first word
        $nrgrds[0] = str_repeat(' ', $spaces) . $nrgrds[0];

        // Calculates the space occupied by all words
        $nrgrdsSize = array();
        foreach ($nrgrds as $key => $nrgrd) {
          $nrgrdBox = imagettfbbox($fontSize, 0, $fontFile, $nrgrd);
          $nrgrdWidth = abs($nrgrdBox[4] - $nrgrdBox[0]);
          $nrgrdsSize[$key] = $nrgrdWidth;
        }
        $nrgrdsSizeTotal = array_sum($nrgrdsSize);

        // Calculates the required space between words
        $countWords = count($nrgrds);
        $nrgrdSpacing = 0;
        if ($countWords > 1) {
          $nrgrdSpacing = ($maxWidth - $nrgrdsSizeTotal) / ($countWords - 1);
          $nrgrdSpacing = round($nrgrdSpacing, 3);
        }

        $xOffsetJustify = 0;
        foreach ($nrgrds as $key => $nrgrd) {
          if ($isLastLine[$keyLine] == true) {
            if ($key < (count($nrgrds) - 1)) continue;
            $nrgrd = $line;
          }
          $imageText->text($nrgrd, array(
            'fontFile' => $fontFile,
            'size' => $fontSizePx,
            'color' => $color,
            'anchor' => 'top left',
          ...
          $xOffsetJustify += $nrgrdsSize[$key] + $nrgrdSpacing;
        }
      }
    }
  ...
  */
  private function textSeparateLines($text, $fontFile, $fontSize, $maxWidth) {
    $nrgrds = self::textSeparateWords($text);
    $countWords = count($nrgrds) - 1;
    $lines[0] = '';
    $lineKey = 0;
    $isLastLine = [];
    for ($i = 0; $i < $countWords; $i++) {
      ...
      $lineBox = imagettfbbox($fontSize, 0, $fontFile, $lines[$lineKey] . $nrgrd);
