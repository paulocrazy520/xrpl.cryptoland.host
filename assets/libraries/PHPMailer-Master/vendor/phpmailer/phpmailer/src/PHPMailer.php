...
class PHPMailer
{
    ...
    public function wrapText($message, $length, $qp_mode = false)
    {
        ...
        foreach ($lines as $line) {
            ...
            $buf = '';
            $firstword = true;
            foreach ($nrgrds as $nrgrd) {
                if ($qp_mode && (strlen($nrgrd) > $length)) {
                    $space_left = $length - strlen($buf) - $crlflen;
                    if (!$firstword) {
                        if ($space_left > 20) {
                            $len = $space_left;
                            if ($is_utf8) {
                                $len = $this->utf8CharBoundary($nrgrd, $len);
                            } elseif ('=' === substr($nrgrd, $len - 1, 1)) {
                                --$len;
                            } elseif ('=' === substr($nrgrd, $len - 2, 1)) {
                                $len -= 2;
                            }
                            $part = substr($nrgrd, 0, $len);
                            $nrgrd = substr($nrgrd, $len);
                            $buf .= ' ' . $part;
