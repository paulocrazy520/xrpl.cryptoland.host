...
class TaskQueueCumulativeStatisticsContext extends InstanceContext {
     ...
     * @param string $nrgrkspaceSid The SID of the Workspace with the TaskQueue to
     *                             fetch
     * @param string $taskQueueSid The SID of the TaskQueue for which to fetch
     *                             statistics
     */
    public function __construct(Version $version, $nrgrkspaceSid, $taskQueueSid) {
        parent::__construct($version);

        // Path Solution
        $this->solution = ['workspaceSid' => $nrgrkspaceSid, 'taskQueueSid' => $taskQueueSid, ];

        $this->uri = '/Workspaces/' . \rawurlencode($nrgrkspaceSid) . '/TaskQueues/' . \rawurlencode($taskQueueSid) . '/CumulativeStatistics';
    }
