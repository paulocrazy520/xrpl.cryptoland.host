...
function NRG_RegisterPoint($post_id, $type, $action = '+', $user_id = 0)
{
    global $nrg, $sqlConnect, $db;
    if ($nrg['config']['point_level_system'] == 0) {
        return false;
    }
    ...
    $points               = 0;
    $dollar_to_point_cost = $nrg['config']['dollar_to_point_cost'];
    switch ($type) {
        case "comments":
            ...
            if ($sql_query_comments->num_rows == 1) {
                $points = $nrg['config']['comments_point'];
            }
            break;
        case "likes":
            if (!NRG_IsWondered($post_id, $user_id)) {
                $points = $nrg['config']['likes_point'];
            }
            break;
        case "dislikes":
            if (!NRG_IsLiked($post_id, $user_id)) {
                $points = $nrg['config']['dislikes_point'];
            }
            break;
        case "wonders":
            if (!NRG_IsLiked($post_id, $user_id)) {
                $points = $nrg['config']['wonders_point'];
            }
            break;
        case "reaction":
            $points = $nrg['config']['reaction_point'];
            break;
        case "createpost":
            $points = $nrg['config']['createpost_point'];
            break;
        case "createblog":
            $points = $nrg['config']['createblog_point'];
            break;
        case "blog_comment":
    ...
    if ($action == '+') {
        ...
        if ($nrg["user"]["is_pro"] && $daily_points > $nrg['config']['pro_day_limit']) {
            return false;
        } elseif ($nrg["user"]["is_pro"] == 0 && $daily_points > $nrg['config']['free_day_limit']) {
            return false;
        }
    } else if ($action == '-') {
        $converted_points  = ($user_data['converted_points'] - $points);
    ...
    $query_one = "";
    if ($nrg['config']['point_allow_withdrawal'] == 1) {
...
function NRG_RegisterProductMedia($id, $media)
{
...
function NRG_RegisterProduct($registration_data)
{
    global $nrg, $sqlConnect;
...
function NRG_GetProduct($id = 0)
{
    ...
    $fetched_data['images']           = NRG_GetProductImages($fetched_data['id']);
    $fetched_data['time_text']        = NRG_Time_Elapsed_String($fetched_data['time']);
    $fetched_data['post_id']          = NRG_GetPostIDFromProdcutID($fetched_data['id']);
    $fetched_data['edit_description'] = NRG_EditMarkup(br2nl($fetched_data['description'], true, false, false));
    $fetched_data['description']      = NRG_Markup($fetched_data['description'], true, false, false);
    if ($nrg['config']['useSeoFrindly'] == 1) {
        $fetched_data['url']            = NRG_SeoLink('index.php?link1=post&id=' . $fetched_data['post_id']) . '_' . NRG_SlugPost($fetched_data['name']);
        $fetched_data['reviews_url']    = NRG_SeoLink('index.php?link1=reviews&id=' . $fetched_data['id']) . '_' . NRG_SlugPost($fetched_data['name']);
        $fetched_data['seo_id']         = $fetched_data['post_id'] . '_' . NRG_SlugPost($fetched_data['name']);
        $fetched_data['reviews_seo_id'] = $fetched_data['id'] . '_' . NRG_SlugPost($fetched_data['name']);
    } else {
        $fetched_data['url']            = NRG_SeoLink('index.php?link1=post&id=' . $fetched_data['post_id']);
        $fetched_data['reviews_url']    = NRG_SeoLink('index.php?link1=reviews&id=' . $fetched_data['id']);
        $fetched_data['seo_id']         = $fetched_data['post_id'];
        $fetched_data['reviews_seo_id'] = $fetched_data['id'];
    }
    //$fetched_data['url']              = NRG_SeoLink('index.php?link1=post&id=' . $fetched_data['post_id']);
    $fetched_data['product_sub_category'] = '';
    if (!empty($fetched_data['sub_category']) && !empty($nrg['products_sub_categories'][$fetched_data['category']])) {
        foreach ($nrg['products_sub_categories'][$fetched_data['category']] as $key => $value) {
    ...
    $fields                 = NRG_GetCustomFields('product');
    if (!empty($fields)) {
    ...
    if ($nrg['loggedin']) {
        $fetched_data['added_to_cart'] = $db->where('product_id', $fetched_data['id'])->where('user_id', $nrg['user']['user_id'])->getValue(T_USERCARD, 'COUNT(*)');
    }
...
function NRG_DeleteProductImage($id)
{
    global $nrg, $sqlConnect;
...
function NRG_GetProductImages($id = 0)
{
    global $nrg, $sqlConnect;
    $data      = array();
    $id        = NRG_Secure($id);
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            ...
            $fetched_data['image_org'] = NRG_GetMedia($fetched_data['image_org']);
...
function NRG_ProductImageData($data = array())
{
    global $nrg, $sqlConnect;
    if (!empty($data['id'])) {
    ...
    if (!empty($data['product_id']) && is_numeric($data['product_id'])) {
        $data['product_id'] = NRG_Secure($data['product_id']);
...
function NRG_MarkPostAsSold($post_id = 0, $product_id = 0)
{
    ...
    if (NRG_IsProductSold($product_id)) {
        return false;
    }
...
function NRG_GetPostIDFromProdcutID($id)
{
    global $sqlConnect;
    if (empty($id) or !is_numeric($id) or $id < 1) {
        return false;
...
function NRG_UpdateProductData($product_id, $update_data)
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    $product_id = NRG_Secure($product_id);
    $post_id    = NRG_GetPostIDFromProdcutID($product_id);
    if (empty($post_id)) {
        return false;
    }
    ...
    foreach ($update_data as $field => $data) {
        $update[] = '`' . $field . '` = \'' . NRG_Secure($data, 1, true, 1) . '\'';
    }
...
function LinkedinSearch($filter_data = array())
{
    ...
    if (!empty($filter_data['experience']) && is_numeric($filter_data['experience']) && $filter_data['experience'] > 0) {
        $experience = NRG_Secure($filter_data['experience']);
...
function NRG_GetProducts($filter_data = array())
{
    global $nrg, $sqlConnect;
    $data      = array();
    ...
    if (!empty($filter_data['after_id'])) {
        if (is_numeric($filter_data['after_id'])) {
            $after_id = NRG_Secure($filter_data['after_id']);
    ...
    if (!empty($filter_data['length'])) {
        $user_lat  = $nrg['user']['lat'];
        $user_lng  = $nrg['user']['lng'];
        $unit      = 6371;
        $query_one = " AND status <> '1'";
        $distance  = NRG_Secure($filter_data['length']);
        if (!empty($filter_data['c_id'])) {
            $category = $filter_data['c_id'];
            $query_one .= " AND `category` = '{$category}'";
        }
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $products           = NRG_GetProduct($fetched_data['id']);
            $products['seller'] = NRG_UserData($fetched_data['user_id']);
            $data[]             = $products;
        }
    }
    return $data;
}
function NRG_AddOption($post_id, $text)
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_GetPostOptions($post_id = 0)
{
    global $sqlConnect;
    if (empty($post_id) or !is_numeric($post_id) or $post_id < 1) {
        return false;
...
function NRG_GetPostIDFromOptionID($id)
{
    global $sqlConnect;
    if (empty($id) or !is_numeric($id) or $id < 1) {
        return false;
...
function NRG_VoteUp($option_id = 0, $user_id = 0)
{
    global $sqlConnect;
    ...
    $user_id   = NRG_Secure($user_id);
    $option_id = NRG_Secure($option_id);
    $post_id   = NRG_GetPostIDFromOptionID($option_id);
    if (empty($post_id)) {
        return false;
    }
    if (NRG_IsPostVoted($post_id, $user_id)) {
        return false;
    }
    if (NRG_IsOptionVoted($option_id, $user_id)) {
        return false;
    }
...
function NRG_IsOptionVoted($option_id, $user_id)
{
    global $nrg, $sqlConnect;
...
function NRG_IsPostVoted($post_id, $user_id)
{
    global $nrg, $sqlConnect;
...
function Ju_GetPercentageOfOptionPost($post_id)
{
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $fetched_data['option_votes'] = NRG_GetVotes($fetched_data['id']);
            $data[]                       = $fetched_data;
        }
    }
    foreach ($data as $key => $value) {
...
function NRG_GetVotes($option_id)
{
    global $nrg, $sqlConnect;
...
function NRG_GetCustomPages()
{
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $data[] = NRG_GetCustomPage($fetched_data['page_name']);
        }
    }
    return $data;
}
function NRG_GetCustomPage($page_name)
{
    global $sqlConnect;
    if (empty($page_name)) {
        return false;
...
function NRG_RegisterNewPage($registration_data)
{
    global $nrg, $sqlConnect;
    if (empty($registration_data)) {
        return false;
...
function NRG_DeleteCustomPage($id)
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_UpdateCustomPageData($id, $update_data)
{
...
function NRG_GetReferrers($user_id = 0)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_UpdateBalance($user_id = 0, $balance = 0, $type = '+', $withdrawal = '')
{
    global $nrg, $sqlConnect;
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 0) {
        return false;
    ...
    $user_id   = NRG_Secure($user_id);
    $balance   = NRG_Secure($balance);
    $c_balance   = NRG_Secure($balance);
    $user_data = NRG_UserData($user_id);
    ...
    if ($type == '+') {
        ...
        $balance = ($user_data['balance'] - $balance);
        if (!empty($withdrawal)) {
            if ($balance == 0 && $nrg['config']['point_allow_withdrawal'] == 1) {
                $wallet         = $user_data['points'] / $nrg['config']['dollar_to_point_cost'];
                ...
                $c_points = $c_balance * $nrg['config']['dollar_to_point_cost'];
...
function NRG_RequestNewPayment($user_id = 0, $amount = 0, $insert_array = array())
{
    ...
    $user_id = NRG_Secure($user_id);
    $amount  = NRG_Secure($amount);
    if (NRG_IsUserPaymentRequested($user_id) === true) {
        return false;
    }
    $user_data   = NRG_UserData($user_id);
    $full_amount = NRG_Secure($user_data['balance']);
    $time        = time();
    $options     = array(
        "iban" => "",
        "country" => "",
...
function NRG_IsUserPaymentRequested($user_id = 0)
{
    global $sqlConnect;
    if (empty($user_id)) {
        return false;
...
function NRG_GetPaymentsHistory($user_id = 0)
{
    global $sqlConnect;
    if (empty($user_id)) {
        return false;
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $data[] = NRG_GetPaymentHistory($fetched_data['id']);
        }
    }
    return $data;
}
function NRG_GetAllPaymentsHistory($type = 0)
{
    global $sqlConnect;
...
function NRG_CountPaymentHistory($id)
{
    global $sqlConnect;
    $data          = array();
    $id            = NRG_Secure($id);
...
function NRG_GetPaymentHistory($id)
{
    global $sqlConnect, $nrg;
    if (empty($id)) {
        return false;
    ...
    if (mysqli_num_rows($sql_query_one)) {
        $fetched_data                 = mysqli_fetch_assoc($sql_query_one);
        $fetched_data['user']         = NRG_UserData($fetched_data['user_id']);
        $fetched_data['total_refs']   = NRG_CountRefs($fetched_data['user_id']);
        $fetched_data['time_text']    = NRG_Time_Elapsed_String($fetched_data['time']);
        $fetched_data['callback_url'] = $nrg['config']['site_url'] . '/' . 'requests.php?f=admincp&paid_user_id=' . $fetched_data['user_id'] . '&paid_ref_id=' . $fetched_data['id'];
        return $fetched_data;
    }
    return false;
}
function NRG_CountRefs($user_id = 0)
{
    global $sqlConnect;
    $data          = array();
    $user_id       = NRG_Secure($user_id);
...
function NRG_InsertBlog($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_RegisterBlogComment($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if ($query) {
        ...
        NRG_RegisterPoint($comment_id, "blog_comment");
        return $comment_id;
    }
    return false;
}
function NRG_RegisterBlogCommentReply($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_GetBlogCommentsCount($id)
{
    global $sqlConnect, $nrg;
    $is_owner = false;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
    $count = 0;
...
function NRG_GetBlogComments($args = array())
{
    global $sqlConnect, $nrg;
    $options   = array(
        "id" => false,
    ...
    $blog_id   = NRG_Secure($args['blog_id']);
    $query_one = '';
    $data      = array();
    if ($offset > 0) {
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $comment = NRG_GetBlogCommentData($fetched_data['id']);
...
function NRG_GetBlogCommLikes($id)
{
    global $sqlConnect, $nrg;
    $id    = NRG_Secure($id);
    $likes = 0;
...
function NRG_GetBlogCommReplyLikes($id)
{
    global $sqlConnect, $nrg;
    $id    = NRG_Secure($id);
    $likes = 0;
...
function NRG_GetBlogCommReplyDisLikes($id)
{
    global $sqlConnect, $nrg;
    $id    = NRG_Secure($id);
    $likes = 0;
...
function NRG_GetBlogCommDisLikes($id)
{
    global $sqlConnect, $nrg;
    $id    = NRG_Secure($id);
    $likes = 0;
...
function NRG_GetBlogCommentReplies($args = array())
{
    global $sqlConnect, $nrg;
    $options   = array(
        "id" => false,
    ...
    $args      = array_merge($options, $args);
    $id        = NRG_Secure($args['id']);
    $comm_id   = NRG_Secure($args['comm_id']);
    $limit     = "";
    $query_one = '';
    $data      = array();
    if ($id && $id > 0 && is_numeric($id)) {
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $comment = NRG_GetBlogCommReplyData($fetched_data['id']);
            if ($comment && !empty($comment)) {
                $data[] = $comment;
            }
        }
...
function NRG_IsBlogCommentOwner($id)
{
    global $sqlConnect, $nrg;
    $is_owner = false;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data) && is_array($fetched_data)) {
            if ($fetched_data['user_id'] == $nrg['user']['id'] || $nrg['user']['admin'] == 1) {
                $is_owner = true;
            }
        }
    }
...
function NRG_IsBlogCommReplyOwner($id)
{
    global $sqlConnect, $nrg;
    $is_owner = false;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
...
function NRG_GetBlogCommentData($id)
{
    global $sqlConnect, $nrg;
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data)) {
            $fetched_data['user_data'] = NRG_UserData($fetched_data['user_id']);
            $fetched_data['is_owner']  = NRG_IsBlogCommentOwner($fetched_data['id']);
            $fetched_data['likes']     = NRG_GetBlogCommLikes($fetched_data['id']);
            $fetched_data['dislikes']  = NRG_GetBlogCommDisLikes($fetched_data['id']);
            $fetched_data['replies']   = NRG_GetBlogCommentReplies(array(
...
function NRG_GetBlogCommReplyData($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data)) {
            $fetched_data['user_data'] = NRG_UserData($fetched_data['user_id']);
            $fetched_data['is_owner']  = NRG_IsBlogCommReplyOwner($fetched_data['id']);
            $fetched_data['likes']     = NRG_GetBlogCommReplyLikes($fetched_data['id']);
            $fetched_data['dislikes']  = NRG_GetBlogCommReplyDisLikes($fetched_data['id']);
            $data                      = $fetched_data;
        }
        return $data;
    }
...
function NRG_IsBlogCommentLikeExists($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_IsBlogCommentDisLikeExists($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_IsBlogCommentReplyDisLikeExists($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_IsBlogCommentReplyLikeExists($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_RemoveBlogCommentLikes($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_RemoveBlogCommentReplyLikes($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_RemoveBlogCommentReplyDisLikes($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_RemoveBlogCommentDisLikes($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_AddBlogCommentLikes($id, $blog)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$blog || !is_numeric($blog)) {
        return false;
    }
    $id      = NRG_Secure($id);
    $blog    = NRG_Secure($blog);
    $comment = NRG_GetBlogCommentData($id);
    $result  = false;
    $user    = $nrg['user']['id'];
    @NRG_RemoveBlogCommentDisLikes($id);
    if ($comment && !empty($comment) && !NRG_IsBlogCommentLikeExists($id)) {
    ...
    } else if ($comment && !empty($comment) && NRG_RemoveBlogCommentLikes($id)) {
        $result = true;
    }
    return $result;
}
function NRG_AddBlogCommReplyLikes($id, $blog)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$blog || $blog < 1) {
        return false;
    }
    $id      = NRG_Secure($id);
    $blog    = NRG_Secure($blog);
    $comment = NRG_GetBlogCommReplyData($id);
    $result  = false;
    $user    = $nrg['user']['id'];
    if ($comment && !empty($comment) && !NRG_IsBlogCommentReplyLikeExists($id)) {
        $sql   = "INSERT INTO " . T_BM_LIKES . "
                    (`id`, `blog_comm_id`,`blog_commreply_id`, `movie_comm_id`, `movie_commreply_id`, `user_id`, `blog_id`)
        ...
        if ($query) {
            $result = true;
            @NRG_RemoveBlogCommentReplyDisLikes($id);
        }
    } else if ($comment && !empty($comment) && NRG_RemoveBlogCommentReplyLikes($id)) {
        $result = true;
    }
    return $result;
}
function NRG_DeleteBlogComment($id, $blog = 0)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !NRG_IsBlogCommentOwner($id)) {
        return false;
    }
    $blog_id = NRG_Secure($blog);
    ...
    NRG_RegisterPoint($id, "blog_comment", "-");
    return mysqli_query($sqlConnect, "DELETE FROM " . T_BLOG_COMM . " WHERE `id` = '$id'");
}
function NRG_DeleteBlogCommReply($id, $blog = 0)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !NRG_IsBlogCommReplyOwner($id) || !$blog || $blog < 1) {
        return false;
    }
    $blog_id = NRG_Secure($blog);
...
function NRG_IsBlogOwner($blog_id = 0, $user_id = 0)
{
    global $sqlConnect, $nrg;
    if (empty($blog_id)) {
        return false;
...
function NRG_UpdateBlog($id = 0, $update_data = array())
{
    global $sqlConnect, $nrg;
    $update = array();
    if ($nrg['loggedin'] == false) {
    ...
    $id = NRG_Secure($id);
    if (NRG_IsBlogOwner($id) === false) {
        return false;
    }
    foreach ($update_data as $field => $data) {
        $update[] = '`' . $field . '` = \'' . NRG_Secure($data, 0, false) . '\'';
    }
    $impload   = implode(', ', $update);
...
function NRG_GetMyBlogs($user = 0, $offset = 0)
{
    global $sqlConnect, $nrg;
    $data = array();
    if ($nrg['loggedin'] == false) {
    ...
    $active = '';
    if ($nrg['config']['blog_approval'] == 1 && !NRG_IsAdmin()) {
        $active = " AND `active` = '1' ";
    }
    $user   = NRG_Secure($user);
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = NRG_GetArticle($fetched_data['id']);
        }
    }
    return $data;
}
function NRG_GetBlogs($args = array())
{
    global $sqlConnect, $nrg;
    $options   = array(
        "category" => false,
    ...
    $offset    = NRG_Secure($args['offset']);
    $limit     = NRG_Secure($args['limit']);
    $category  = NRG_Secure($args['category']);
    $order_by  = NRG_Secure($args['order_by']);
    $id        = NRG_Secure($args['id']);
    $user_id   = NRG_Secure($args['user_id']);
    $keyword   = NRG_Secure($args['keyword']);
...
function NRG_DeleteMyBlog($id = 0)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if ($thumbnail) {
        ...
        if ($path['thumbnail'] != 'upload/photos/d-blog.jpg') {
            if (file_exists($path['thumbnail'])) {
                ...
                @NRG_DeleteFromToS3($path['thumbnail']);
            }
        }
    }
    ...
    if ($sql_query_one) {
        ...
        $delete_post   = NRG_DeletePost($mysqli['id']);
    }
    return $sql_query_one;
}
function NRG_GetArticle($id = 0)
{
    ...
    if (mysqli_num_rows($sql_query_one)) {
        $fetched_data = mysqli_fetch_assoc($sql_query_one);
        if (!empty($fetched_data)) {
            $fetched_data['author']     = NRG_UserData($fetched_data['user']);
            $fetched_data['thumbnail']  = NRG_GetMedia($fetched_data['thumbnail']);
            $fetched_data['tags_array'] = @explode(',', $fetched_data['tags']);
            if ($nrg['config']['useSeoFrindly'] == 1) {
                $fetched_data['url'] = NRG_SeoLink('index.php?link1=read-blog&id=' . $fetched_data['id'] . '_' . NRG_SlugPost($fetched_data['title']));
            } else {
                $fetched_data['url'] = NRG_SeoLink('index.php?link1=read-blog&id=' . $fetched_data['id']);
            }
            $fetched_data['author']        = NRG_UserData($fetched_data['user']);
            $fetched_data['category_link'] = NRG_SeoLink('index.php?link1=blog-category&id=' . $fetched_data['category']);
            $fetched_data['category_name'] = '';
            $fetched_data['is_post_admin'] = false;
            if ($nrg['loggedin'] == true) {
                $fetched_data['is_post_admin'] = ($fetched_data['user'] == $nrg['user']['id']) ? true : false;
            }
            if (!empty($nrg['blog_categories'][$fetched_data['category']])) {
                $fetched_data['category_name'] = $nrg['blog_categories'][$fetched_data['category']];
            }
            if ($nrg['config']['second_post_button'] == 'reaction') {
                $post = $db->where('blog_id', $fetched_data['id'])->getOne(T_POSTS);
                if (!empty($post)) {
                    $fetched_data['reaction'] = NRG_GetPostReactionsTypes($post->id);
                }
            }
        }
        return $fetched_data;
...
function NRG_SearchBlogs($args = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
// *** NRG_Wonder Forum ***  //
function NRG_GetForumSec($args = array())
{
    global $sqlConnect, $nrg;
    $options   = array(
        "id" => false,
    ...
    $id        = NRG_Secure($args['id']);
    $limit     = NRG_Secure($args['limit']);
    $search    = NRG_Secure($args['search']);
    $keyword   = NRG_Secure($args['keyword']);
    $forums    = NRG_Secure($args['forums']);
    $order_by  = NRG_Secure($args['order_by']);
    $query_one = "";
    if ($offset > 0) {
        $query_one .= " AND `id` < {$offset} AND `id` <> {$offset} ";
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $fetched_data['section_name_lang'] = NRG_ReplaceText($fetched_data['section_name']);
            $fetched_data['description_lang'] = NRG_ReplaceText($fetched_data['description']);
            if ($forums) {
                $fetched_data['forums'] = NRG_GetForums(array(
...
function NRG_GetForums($args = array())
{
    global $sqlConnect;
    $options   = array(
        "section" => false,
    ...
    $args      = array_merge($options, $args);
    $offset    = NRG_Secure($args['offset']);
    $section   = NRG_Secure($args['section']);
    $limit     = NRG_Secure($args['limit']);
    $search    = NRG_Secure($args['search']);
    $keyword   = NRG_Secure($args['keyword']);
    $order_by  = NRG_Secure($args['order_by']);
    ...
    while ($fetched_data = mysqli_fetch_assoc($sql_queryset)) {
        $fetched_data['name_lang'] = NRG_ReplaceText($fetched_data['name']);
        $fetched_data['description_lang'] = NRG_ReplaceText($fetched_data['description']);
        $fetched_data['posts'] = NRG_GetTotalThreads(array(
...
function NRG_GetShortTitle($text = false, $preview = false, $len = 40)
{
    if (!$text) {
        return false;
    }
...
function NRG_GetForumInfo($fid)
{
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $fetched_data['name_lang'] = NRG_ReplaceText($fetched_data['name']);
            $fetched_data['description_lang'] = NRG_ReplaceText($fetched_data['description']);
            $data['threads'] = NRG_GetForumThreads(array(
                "forum" => $fetched_data['id'],
                "limit" => 10
            ));
            $data['forum']   = $fetched_data;
            $data['forums']  = NRG_GetForums();
        }
    }
    return $data;
}
function NRG_GetForum($fid)
{
    global $sqlConnect;
    if (!$fid || !is_numeric($fid)) {
        return array();
    ...
    if (mysqli_num_rows($sql_query_one)) {
        ...
        if (!empty($fetched_data)) {
            $fetched_data['name_lang'] = NRG_ReplaceText($fetched_data['name']);
            $fetched_data['description_lang'] = NRG_ReplaceText($fetched_data['description']);
            $fetched_data['name'] = NRG_GetShortTitle($fetched_data['name'], true);
        }
        return $fetched_data;
    }
    return false;
}
function NRG_GetForumThreads($args = array())
{
    global $sqlConnect;
    $options   = array(
        "forum" => false,
    ...
    $offset    = NRG_Secure($args['offset']);
    $search    = NRG_Secure($args['search']);
    $subject   = NRG_Secure($args['subject']);
    $post      = NRG_Secure($args['post']);
    $limit     = NRG_Secure($args['limit']);
    $forum     = NRG_Secure($args['forum']);
    $order_by  = NRG_Secure($args['order_by']);
    $id        = NRG_Secure($args['id']);
    $user      = NRG_Secure($args['user']);
    $preview   = NRG_Secure($args['preview']);
    $query_one = "";
    $ordering  = "";
    if ($offset > 0) {
        $query_one .= " AND `id` < {$offset} AND `id` <> {$offset} ";
    ...
    $data         = array();
    while ($fetched_data = mysqli_fetch_assoc($sql_queryset)) {
        $fetched_data['user_data']        = NRG_UserData($fetched_data['user']);
        $fetched_data['url']              = NRG_SeoLink("index.php?link1=showthread&tid=" . $fetched_data['id']);
        $fetched_data['author_url']       = NRG_SeoLink("index.php?link1=timeline&u=" . $fetched_data['user_data']['username']);
        $fetched_data['orginal_headline'] = $fetched_data['headline'];
        $fetched_data['headline']         = NRG_GetShortTitle($fetched_data['headline'], $preview);
        $fetched_data['edit_url']         = NRG_SeoLink('index.php?link1=edithread&tid=' . $fetched_data['id']);
        if (empty($args['threadreplies'])) {
            $fetched_data['threadreplies'] = NRG_GetThreadReplies(array(
                "thread_id" => $fetched_data['id']
            ));
        }
        $fetched_data['replies'] = NRG_GetTotalReplies(array(
...
function NRG_GetMyReplies($args = array())
{
    global $sqlConnect, $nrg;
    $options   = array(
        "offset" => 0,
    ...
    $data         = array();
    while ($fetched_data = mysqli_fetch_assoc($sql_queryset)) {
        $fetched_data['user_data']    = NRG_UserData($fetched_data['poster_id']);
        $fetched_data['forum']        = NRG_GetForum($fetched_data['forum_id']);
        $fetched_data['edit_url']     = NRG_SeoLink("index.php?link1=editreply&tid=" . $fetched_data['id']);
        $fetched_data['url']          = NRG_SeoLink("index.php?link1=showthread&tid=" . $fetched_data['thread_id']);
        $fetched_data['post_subject'] = NRG_GetShortTitle($fetched_data['post_subject']);
        $data[]                       = $fetched_data;
    }
    return $data;
}
function NRG_GetThreadReplies($args = array())
{
    global $sqlConnect, $nrg;
    $options   = array(
        "thread_id" => false,
    ...
    $forum     = NRG_Secure($args['forum']);
    $limit     = NRG_Secure($args['limit']);
    $thread_id = NRG_Secure($args['thread_id']);
    $limit     = NRG_Secure($args['limit']);
    $order_by  = NRG_Secure($args['order_by']);
    $id        = NRG_Secure($args['id']);
    $poster_id = NRG_Secure($args['user']);
    $query_one = "";
    if ($thread_id) {
    ...
    while ($fetched_data = mysqli_fetch_assoc($sql_queryset)) {
        $fetched_data['user_data'] = NRG_UserData($fetched_data['poster_id']);
        $fetched_data['is_owner']  = ($fetched_data['poster_id'] == $nrg['user']['id']) ? true : false;
        $fetched_data['is_admin']  = ($nrg['user']['admin'] == 1) ? true : false;
        $fetched_data['reply-url'] = NRG_SeoLink("index.php?link1=threadquote&tid=" . $fetched_data['id']);
        $fetched_data['edit-url']  = NRG_SeoLink("index.php?link1=editreply&tid=" . $fetched_data['id']);
        if ($forum) {
            $fetched_data['forum'] = NRG_GetForum($fetched_data['forum_id']);
        }
        $data[] = $fetched_data;
...
function NRG_SearchThreadReplies($args = array())
{
    global $sqlConnect, $nrg;
    $options   = array(
        "thread_id" => false,
...
function NRG_IsReplyOwner($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (!empty($fetched_data)) {
        if ($fetched_data['poster_id'] == $nrg['user']['id'] || $nrg['user']['admin'] == 1) {
            return true;
        }
    }
    return false;
}
function NRG_GetForumUsers($args = array())
{
    global $nrg, $sqlConnect;
    $data      = array();
    $options   = array(
    ...
    $offset    = NRG_Secure($args['offset']);
    $limit     = NRG_Secure($args['limit']);
    $key       = NRG_Secure($args['key']);
    $name      = NRG_Secure($args['name']);
    $id        = NRG_Secure($args['id']);
    $query_one = "";
    if ($offset > 0) {
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $data_user                = NRG_UserData($fetched_data['user_id']);
            $data_user['forum_posts'] = NRG_GetTotalThreads(array(
...
function NRG_IsThreadOwner($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    $fetched_data = mysqli_fetch_assoc($sql_queryset);
    if (!empty($fetched_data)) {
        if ($fetched_data['user'] == $nrg['user']['id'] || $nrg['user']['admin'] == 1) {
            return true;
        }
    }
    return false;
}
function NRG_AddTopic($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_AddForumSection($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || NRG_IsAdmin() == false) {
        return false;
    }
    $fields = '`' . implode('`, `', array_keys($registration_data)) . '`';
    $data   = '\'' . implode('\', \'', $registration_data) . '\'';
...
function NRG_AddForum($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || NRG_IsAdmin() == false) {
        return false;
...
function NRG_ThreadReply($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_BbcodeToHtml($bbtext)
{
    $bbtags     = array(
        '[paragraph]' => '<p>',
        '[/paragraph]' => '</p>',
...
function NRG_ThreadUpdate($id = 0, $update_data = array())
{
    global $sqlConnect, $nrg;
    $update = array();
    if ($nrg['loggedin'] == false) {
...
function NRG_UpdateTopic($id = 0, $update_data = array())
{
    global $sqlConnect, $nrg;
    $update = array();
    if ($nrg['loggedin'] == false) {
...
function NRG_DeleteThreadReply($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_DeleteForumThread($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_DeleteForum($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false && NRG_IsAdmin() == false) {
        return false;
...
function NRG_DeleteForumSection($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false && NRG_IsAdmin() == false) {
        return false;
...
function NRG_AddThreadView($id = false)
{
    global $sqlConnect;
    if (!$id || !is_numeric($id)) {
        return false;
...
function NRG_UpdateThreadLastPostTime($id = false)
{
    global $sqlConnect;
    if (!$id || !is_numeric($id)) {
        return false;
...
function NRG_GetTotalThreads($args = array())
{
    global $sqlConnect;
    $options = array(
        "forum" => false,
...
function NRG_GetTotalReplies($args = array())
{
    global $sqlConnect;
    $options = array(
        "thread" => false,
...
function NRG_NotificationWebPushNotifier()
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    }
    if ($nrg['config']['push'] == 0) {
        return false;
    }
    if ($nrg['config']['android_push_native'] == 0 && $nrg['config']['ios_push_native'] == 0 && $nrg['config']['web_push'] == 0) {
        return false;
    }
    $user_id   = NRG_Secure($nrg['user']['user_id']);
    $to_ids    = array();
    ...
    if (mysqli_num_rows($query_get) > 0) {
        while ($sql_get_notification_for_push = mysqli_fetch_assoc($query_get)) {
            $get_session_data = NRG_GetSessionDataFromUserID($sql_get_notification_for_push['recipient_id']);
            if (empty($get_session_data) || !empty($get_session_data)) {
                ...
                $to_data         = NRG_UserData($sql_get_notification_for_push['recipient_id']);
                $ids             = array();
                if (!empty($to_data['android_n_device_id']) || !empty($to_data['ios_n_device_id']) || !empty($to_data['web_device_id'])) {
                    ...
                    if (!empty($to_data['web_device_id']) && empty($to_data['ios_n_device_id']) && empty($to_data['android_n_device_id'])) {
                        if ($nrg['config']['web_push'] == 0) {
                            return false;
                        }
                    }
                    $send_array                                 = array();
                    $lang                                       = $nrg['lang'] = NRG_LangsFromDB($to_data['language']);
                    ...
                    if (!empty($sql_get_notification_for_push['type'])) {
                        if ($sql_get_notification_for_push['type'] == "reaction") {
                            if ($notificationText == "post") {
                                $sql_get_notification_for_push['type_text'] .= $nrg['lang']['reacted_to_your_post'];
                            } else if ($notificationText == "comment") {
                                $sql_get_notification_for_push['type_text'] .= $nrg['lang']['reacted_to_your_comment'];
                            } else if ($notificationText == "replay") {
                                $sql_get_notification_for_push['type_text'] .= $nrg['lang']['reacted_to_your_replay'];
                            }
                        }
                        if ($sql_get_notification_for_push['type'] == "following") {
                            $sql_get_notification_for_push['type_text'] .= $nrg['lang']['followed_you'];
                        }
                        if ($sql_get_notification_for_push['type'] == 'comment_mention') {
                            $sql_get_notification_for_push['type_text'] .= $nrg['lang']['comment_mention'];
                        }
                        if ($sql_get_notification_for_push['type'] == 'post_mention') {
                            $sql_get_notification_for_push['type_text'] .= $nrg['lang']['post_mention'];
                        }
                        if ($sql_get_notification_for_push['type'] == 'liked_post') {
                            $sql_get_notification_for_push['type_text'] = str_replace($orginal_txt, $replaced_txt, $nrg['lang']['liked_post']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'wondered_post') {
                            $lang_type                                  = ($nrg['config']['second_post_button'] == 'wonder') ? $nrg['lang']['wondered_post'] : $nrg['lang']['disliked_post'];
                        ...
                        if ($sql_get_notification_for_push['type'] == 'share_post') {
                            $sql_get_notification_for_push['type_text'] = str_replace($orginal_txt, $replaced_txt, $nrg['lang']['share_post']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'comment') {
                            $sql_get_notification_for_push['type_text'] = str_replace($orginal_txt, $replaced_txt, $nrg['lang']['commented_on_post']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'comment_reply') {
                            $sql_get_notification_for_push['type_text'] = str_replace('{comment}', $sql_get_notification_for_push['text'], $nrg['lang']['replied_to_comment']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'comment_reply_mention') {
                            $sql_get_notification_for_push['type_text'] = str_replace('{comment}', $sql_get_notification_for_push['text'], $nrg['lang']['comment_reply_mention']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'also_replied') {
                            $sql_get_notification_for_push['type_text'] = str_replace('{comment}', $sql_get_notification_for_push['text'], $nrg['lang']['also_replied']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'liked_comment') {
                            $sql_get_notification_for_push['type_text'] = str_replace('{comment}', $sql_get_notification_for_push['text'], $nrg['lang']['liked_comment']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'wondered_comment') {
                            $lang_type                                  = ($nrg['config']['second_post_button'] == 'wonder') ? $nrg['lang']['wondered_comment'] : $nrg['lang']['disliked_comment'];
                        ...
                        if ($sql_get_notification_for_push['type'] == 'liked_reply_comment') {
                            $sql_get_notification_for_push['type_text'] = str_replace('{comment}', $sql_get_notification_for_push['text'], $nrg['lang']['liked_reply_comment']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'wondered_reply_comment') {
                            $lang_type                                  = ($nrg['config']['second_post_button'] == 'wonder') ? $nrg['lang']['wondered_reply_comment'] : $nrg['lang']['disliked_reply_comment'];
                            $sql_get_notification_for_push['type_text'] = str_replace('{comment}', $sql_get_notification_for_push['text'], $lang_type);
                        }
                        if ($sql_get_notification_for_push['type'] == 'profile_wall_post') {
                            $sql_get_notification_for_push['type_text'] = $nrg['lang']['posted_on_timeline'];
                        }
                        if ($sql_get_notification_for_push['type'] == 'visited_profile') {
                            $sql_get_notification_for_push['type_text'] = $nrg['lang']['profile_visted'];
                        }
                        if ($sql_get_notification_for_push['type'] == 'liked_page') {
                            $page                                       = NRG_PageData($sql_get_notification_for_push['page_id']);
                            $sql_get_notification_for_push['type_text'] = str_replace('{page_name}', $page['name'], $nrg['lang']['liked_page']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'joined_group') {
                            $group                                      = NRG_GroupData($sql_get_notification_for_push['group_id']);
                            $sql_get_notification_for_push['type_text'] = str_replace('{group_name}', $group['name'], $nrg['lang']['joined_group']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'accepted_invite') {
                            $page_id                                    = @end(explode('/', $sql_get_notification_for_push['url']));
                            $page                                       = NRG_PageData(NRG_PageIdFromPagename($page_id));
                            $sql_get_notification_for_push['type_text'] = str_replace('{page_name}', $page['name'], $nrg['lang']['accepted_invited_page']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'invited_page') {
                            $page_id                                    = @end(explode('/', $sql_get_notification_for_push['url']));
                            $page                                       = NRG_PageData(NRG_PageIdFromPagename($page_id));
                            $sql_get_notification_for_push['type_text'] = str_replace('{page_name}', $page['name'], $nrg['lang']['invited_page']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'accepted_join_request') {
                            $group_id                                   = @end(explode('/', $sql_get_notification_for_push['url']));
                            $group                                      = NRG_GroupData(NRG_GroupIdFromGroupname($group_id));
                            $sql_get_notification_for_push['type_text'] = str_replace('{group_name}', $group['name'], $nrg['lang']['accepted_join_request']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'added_you_to_group') {
                            $group_id                                   = @end(explode('/', $sql_get_notification_for_push['url']));
                            $group                                      = NRG_GroupData(NRG_GroupIdFromGroupname($group_id));
                            $sql_get_notification_for_push['type_text'] = str_replace('{group_name}', $group['name'], $nrg['lang']['added_you_to_group']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'requested_to_join_group') {
                            $sql_get_notification_for_push['type_text'] = $nrg['lang']['requested_to_join_group'];
                        }
                        if ($sql_get_notification_for_push['type'] == 'interested_event') {
                            $event_data                                 = NRG_EventData($sql_get_notification_for_push['event_id']);
                            $sql_get_notification_for_push['type_text'] = str_replace('{event_name}', $event_data['name'], $nrg['lang']['is_interested']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'going_event') {
                            $event_data                                 = NRG_EventData($sql_get_notification_for_push['event_id']);
                            $sql_get_notification_for_push['type_text'] = str_replace('{event_name}', $event_data['name'], $nrg['lang']['is_going']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'invited_event') {
                            $event_data                                 = NRG_EventData($sql_get_notification_for_push['event_id']);
                            $sql_get_notification_for_push['type_text'] = str_replace('{event_name}', $event_data['name'], $nrg['lang']['invited_you_event']);
                        }
                        if ($sql_get_notification_for_push['type'] == 'forum_reply') {
                            $sql_get_notification_for_push['type_text'] = $nrg['lang']['replied_to_topic'];
                        }
                        if ($sql_get_notification_for_push['type'] == 'accepted_request') {
                            if ($nrg['config']['connectivitySystem'] == 1) {
                                $sql_get_notification_for_push['type_text'] = $nrg['lang']['accepted_friend_request'];
                            } else {
                                $sql_get_notification_for_push['type_text'] = $nrg['lang']['accepted_follow_request'];
                            }
                        }
                        ...
                        if ($sql_get_notification_for_push['type'] == 'page_admin') {
                            $sql_get_notification_for_push['type_text'] = $nrg['lang']['added_page_admin'];
                            $sql_get_notification_for_push['url']       = $sql_get_notification_for_push['url'];
                        }
                        if ($sql_get_notification_for_push['type'] == 'group_admin') {
                            $sql_get_notification_for_push['type_text'] = $nrg['lang']['added_group_admin'];
                            $sql_get_notification_for_push['url']       = $sql_get_notification_for_push['url'];
                        }
                    ...
                    if ($nrg['config']['android_push_native'] == 1 && !empty($to_data['android_n_device_id'])) {
                        $send_array['send_to']                                      = array(
                            $to_data['android_n_device_id']
                        );
                        $send_array['notification']['notification_title']           = $nrg['user']['name'];
                        $send_array['notification']['notification_image']           = $nrg['user']['avatar'];
                        $send_array['notification']['notification_data']['user_id'] = $user_id;
                        $send                                                       = NRG_SendPushNotification($send_array, 'android_native');
                    }
                    if ($nrg['config']['ios_push_native'] == 1 && !empty($to_data['ios_n_device_id'])) {
                        $send_array['send_to']                                      = array(
                        ...
                        $send_array['notification']['notification_image']           = $nrg['user']['avatar'];
                        $send_array['notification']['notification_data']['user_id'] = $user_id;
                        $send                                                       = NRG_SendPushNotification($send_array, 'ios_native');
                    }
                    if ($nrg['config']['web_push'] == 1 && !empty($to_data['web_device_id'])) {
                        $send_array['send_to']                                      = array(
                        ...
                        $send_array['notification']['notification_image']           = $nrg['user']['avatar'];
                        $send_array['notification']['notification_data']['user_id'] = $user_id;
                        $send                                                       = NRG_SendPushNotification($send_array, 'web');
                    }
                }
...
function NRG_MessagesPushNotifier()
{
    global $sqlConnect, $nrg, $db;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if ($nrg['config']['android_push_messages'] == 0 && $nrg['config']['ios_push_messages'] == 0) {
        return false;
    }
    $user_id   = NRG_Secure($nrg['user']['user_id']);
    $to_ids    = array();
    ...
    if (mysqli_num_rows($query_get) > 0) {
        while ($sql_get_messages_for_push = mysqli_fetch_assoc($query_get)) {
            if (!in_array($sql_get_messages_for_push['to_id'], $to_ids)) {
                $get_session_data = NRG_GetSessionDataFromUserID($sql_get_messages_for_push['to_id']);
                if (empty($get_session_data)) {
                    ...
                    if ($send_notify) {
                        ...
                        $to_data           = NRG_UserData($sql_get_messages_for_push['to_id']);
                        ...
                        if (!empty($to_data['android_m_device_id']) && $nrg['config']['android_push_messages'] != 0) {
                            $send_array = array(
                                'send_to' => array(
                                ...
                                'notification' => array(
                                    ...
                                    'notification_title' => $nrg['user']['name'],
                                    'notification_image' => $nrg['user']['avatar'],
                                    'notification_data' => $notification_data
                                )
                            );
                            $send       = NRG_SendPushNotification($send_array, 'android_messenger');
                            if ($send) {
                                $query_get_messages_for_push = mysqli_query($sqlConnect, "UPDATE " . T_MESSAGES . " SET `notification_id` = '$send' WHERE `id` = '$message_id'");
                            }
                        }
                        if (!empty($to_data['ios_m_device_id']) && $nrg['config']['ios_push_messages'] != 0) {
                            $send_array = array(
                                'send_to' => array(
                            ...
                            $send       = NRG_SendPushNotification($send_array, 'ios_messenger');
                            if ($send) {
                                $query_get_messages_for_push = mysqli_query($sqlConnect, "UPDATE " . T_MESSAGES . " SET `notification_id` = '$send' WHERE `id` = '$message_id'");
                            }
                        }
...
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id) {
        return false;
    }
    $user  = ($user && is_numeric($user)) ? $user : $nrg['user']['id'];
...
function NRG_InsertEvent($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if ($query) {
        ...
        $register_post = NRG_RegisterPost(array(
            'user_id' => NRG_Secure($nrg['user']['user_id']),
            'time' => time(),
            'postPrivacy' => '0',
            'page_event_id' => $id
...
function NRG_UpdateEvent($id = 0, $update_data = array())
{
    global $sqlConnect, $nrg;
    $update = array();
    if ($nrg['loggedin'] == false) {
...
function NRG_EventGoingExists($event_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_EventInterestedExists($event_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_EventInvitedExists($event_id, $user_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_TotalInvitedUsers($event_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_TotalGoingUsers($event_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_TotalInterestedUsers($event_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_AddEventGoingUsers($event_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (NRG_EventGoingExists($event_id)) {
        return false;
    }
    $user_id    = $nrg['user']['id'];
    $event_id   = NRG_Secure($event_id);
    $event_data = NRG_EventData($event_id);
    ...
    if ($query) {
        ...
        $notification_data_array = array(
            ...
            'type' => 'going_event',
            'event_id' => $event_id,
            'url' => 'index.php?link1=timeline&u=' . $nrg['user']['username']
        );
        NRG_RegisterNotification($notification_data_array);
    }
    return $query;
}
function NRG_AddEventInvitedUsers($event_id, $user_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (NRG_EventInvitedExists($event_id, $user_id)) {
        return false;
    }
    $invited_id = NRG_Secure($user_id);
    $inviter_id = $nrg['user']['id'];
    $event_id   = NRG_Secure($event_id);
    $sql        = "INSERT INTO " . T_EVENTS_INV . " (`id`, `event_id`, `inviter_id`,`invited_id`)
                                      VALUES (NULL, '$event_id', '$inviter_id','$invited_id')";
    return mysqli_query($sqlConnect, $sql);
}
function NRG_AddEventInterestedUsers($event_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (NRG_EventInterestedExists($event_id)) {
        return false;
    }
    $user_id    = $nrg['user']['id'];
    $event_id   = NRG_Secure($event_id);
...
function NRG_UnsetEventInterestedUsers($event_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_UnsetEventGoingUsers($event_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_GetEvents($args = array())
{
    global $sqlConnect, $nrg;
    // if ($nrg['loggedin'] == false) {
    //     return false;
    ...
    if ($nrg['config']['events_visibility'] == 1) {
        $user = $nrg['user']['id'];
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            $fetched_data['start_date'] = date($nrg['config']['date_style'], strtotime($fetched_data['start_date'] . $fetched_data['start_time']));
            $fetched_data['cover']      = NRG_GetMedia($fetched_data['cover']);
            $fetched_data['url']        = NRG_SeoLink("index.php?link1=show-event&eid=" . $fetched_data['id']);
            $data[]                     = $fetched_data;
        }
    }
    return $data;
}
function NRG_GetSuggestedEvents($args = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_GetGoingEvents($offset = 0, $limit = 10)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        if ($query && !empty($query)) {
            while ($fetched_data = mysqli_fetch_assoc($query)) {
                $data[] = NRG_EventData($fetched_data['event_id']);
            }
        }
    }
    return $data;
}
function NRG_GetInvitedEvents($offset = 0, $limit = 10)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_GetInterestedEvents($offset = 0, $limit = 10)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $event = NRG_EventData($fetched_data['event_id']);
...
function NRG_GetInterestedEventsUsers($event_id, $offset = 0, $limit = 0)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $event          = NRG_UserData($fetched_data['user_id']);
...
function NRG_GetGoingEventsUsers($event_id, $offset = 0, $limit = 0)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_GetPastEvents($offset = 0, $limit = 10)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_GetMyEvents($offset = 0, $limit = 10)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_DeleteApp($id = false)
{
    global $sqlConnect, $nrg;
    $result = false;
    if ($nrg['loggedin'] == false) {
...
function NRG_DeleteEvent($id = false)
{
    global $sqlConnect, $nrg;
    $result = false;
    if ($nrg['loggedin'] == false) {
...
function NRG_EventData($id = false)
{
    global $sqlConnect, $nrg;
    // if ($nrg['loggedin'] == false || !$id || !is_numeric($id)) {
    //     return false;
    // }
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_EVENTS . "  WHERE `id` = '$id'");
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data)) {
            ...
            $fetched_data['end_date']        = date($nrg['config']['date_style'], strtotime($fetched_data['end_date']));
            $fetched_data['url']             = NRG_SeoLink("index.php?link1=show-event&eid=" . $fetched_data['id']);
            return $fetched_data;
        }
    }
...
function NRG_RegsiterEventInvite($user_id, $event_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (!Is_EventOwner($event_id, $user_id) && NRG_AddEventInvitedUsers($event_id, $user_id)) {
        $notification_data_array = array(
...
function DetermineUserLang()
{
    ...
    if (array_key_exists($lang, $nrg_langs)) {
        $language = $nrg_langs[$lang];
    }
    return $language;
}
// *** Movies ***
function NRG_InsertFilm($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_UpdateFilm($id = 0, $update_data = array())
{
    global $sqlConnect, $nrg;
    $update = array();
    if ($nrg['loggedin'] == false) {
...
function NRG_AddBlogCommReplyDisLikes($id, $blog)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$blog || $blog < 1) {
        return false;
    ...
    $result  = false;
    $user    = $nrg['user']['id'];
    if ($comment && !empty($comment) && !NRG_IsBlogCommentReplyDisLikeExists($id)) {
        ...
        if ($query) {
            $result = true;
            @NRG_RemoveBlogCommentReplyLikes($id);
        }
    } else if ($comment && !empty($comment) && NRG_RemoveBlogCommentReplyDisLikes($id)) {
        $result = true;
    }
    return $result;
}
function NRG_AddBlogCommentDisLikes($id, $blog)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$blog || $blog < 1) {
        return false;
    ...
    $result  = false;
    $user    = $nrg['user']['id'];
    @NRG_RemoveBlogCommentLikes($id);
    if ($comment && !empty($comment) && !NRG_IsBlogCommentDisLikeExists($id)) {
        $sql   = "INSERT INTO " . T_BM_DISLIKES . "
                    (`id`, `blog_comm_id`,`blog_commreply_id`, `movie_comm_id`,`movie_commreply_id`, `user_id`,`blog_id`)
                        VALUES (NULL, '$id', '0','0','0', '$user','$blog')";
        $query = mysqli_query($sqlConnect, $sql);
    ...
    } else if ($comment && !empty($comment) && NRG_RemoveBlogCommentDisLikes($id)) {
        $result = true;
    }
    return $result;
}
function NRG_GetMovies($args = array())
{
    global $sqlConnect, $nrg;
    // if ($nrg['loggedin'] == false) {
    //     return false;
    ...
    $offset  = $args['offset'];
    $limit   = $args['limit'];
    $genre   = NRG_Secure($args['genre']);
    $id      = NRG_Secure($args['id']);
    $country = NRG_Secure($args['country']);
    $sub_sql = "";
    $total   = "";
    if ($offset && $offset > 0) {
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['cover']  = NRG_GetMedia($fetched_data['cover']);
            $fetched_data['source'] = NRG_GetMedia($fetched_data['source']);
            $fetched_data['url']    = NRG_SeoLink("index.php?link1=watch-film&film-id=" . $fetched_data['id']) . '_' . NRG_SlugPost($fetched_data['name']);
            $data[]                 = $fetched_data;
        }
    }
    return $data;
}
function NRG_GetRecommendedFilms()
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_GetMtwFilms($limit = 26)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_SearchFilms($key)
{
    global $sqlConnect, $nrg;
    // if ($nrg['loggedin'] == false || !$key) {
    //     return false;
    // }
    $data  = array();
    $key   = NRG_Secure($key);
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['url']   = NRG_SeoLink("index.php?link1=watch-film&film-id=" . $fetched_data['id']) . '_' . NRG_SlugPost($fetched_data['name']);
            $fetched_data['name']  = NRG_GetShortTitle($fetched_data['name']);
            $fetched_data['cover'] = NRG_GetMedia($fetched_data['cover']);
            $data[]                = $fetched_data;
        }
    }
...
function NRG_DeleteFilm($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_numeric($id) || ($nrg['user']['admin'] != 1 && NRG_IsModerator() == false)) {
        return false;
    }
    $id  = NRG_Secure($id);
...
function NRG_RegisterMovieComment($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_RegisterMovieCommentReply($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_GetMovieCommentsCount($id)
{
    global $sqlConnect, $nrg;
    $is_owner = false;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
...
function NRG_GetMovieComments($args = array())
{
    global $sqlConnect, $nrg;
    $options   = array(
        "id" => false,
    ...
    $offset    = NRG_Secure($args['offset']);
    $id        = NRG_Secure($args['id']);
    $movie_id  = NRG_Secure($args['movie_id']);
    $query_one = '';
    $data      = array();
    if ($offset > 0) {
        $query_one .= " AND `id` < {$offset} AND `id` <> {$offset} ";
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $comment = NRG_GetMovieCommentData($fetched_data['id']);
            if ($comment && !empty($comment)) {
                $data[] = $comment;
            }
        }
...
function NRG_GetMovieCommentReplies($args = array())
{
    global $sqlConnect, $nrg;
    $options   = array(
        "id" => false,
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $comment = NRG_GetMovieCommReplyData($fetched_data['id']);
            if ($comment && !empty($comment)) {
                $data[] = $comment;
            }
        }
...
function NRG_IsMovieCommentOwner($id)
{
    global $sqlConnect, $nrg;
    $is_owner = false;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
...
function NRG_IsMovieCommReplyOwner($id)
{
    global $sqlConnect, $nrg;
    $is_owner = false;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
...
function NRG_GetMovieCommentData($id)
{
    global $sqlConnect, $nrg;
    if (!$id || !is_numeric($id) || $id < 1) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data)) {
            $fetched_data['user_data'] = NRG_UserData($fetched_data['user_id']);
            $fetched_data['is_owner']  = NRG_IsMovieCommentOwner($fetched_data['id']);
            $fetched_data['likes']     = NRG_GetMovieCommLikes($fetched_data['id']);
            $fetched_data['dislikes']  = NRG_GetMovieCommDisLikes($fetched_data['id']);
            $fetched_data['replies']   = NRG_GetMovieCommentReplies(array(
                'comm_id' => $fetched_data['id']
            ));
            $data                      = $fetched_data;
        }
...
function NRG_GetMovieCommReplyData($id)
{
    global $sqlConnect, $nrg;
    if (!$id || !is_numeric($id) || $id < 1) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data)) {
            $fetched_data['user_data'] = NRG_UserData($fetched_data['user_id']);
            $fetched_data['is_owner']  = NRG_IsMovieCommReplyOwner($fetched_data['id']);
            $fetched_data['likes']     = NRG_GetMovieCommReplyLikes($fetched_data['id']);
            $fetched_data['dislikes']  = NRG_GetMovieCommReplyDisLikes($fetched_data['id']);
            $data                      = $fetched_data;
        }
        return $data;
    }
...
function NRG_GetMovieCommLikes($id)
{
    global $sqlConnect, $nrg;
    $id    = NRG_Secure($id);
    $likes = 0;
...
function NRG_GetMovieCommDisLikes($id)
{
    global $sqlConnect, $nrg;
    $id    = NRG_Secure($id);
    $likes = 0;
...
function NRG_GetMovieCommReplyDisLikes($id)
{
    global $sqlConnect, $nrg;
    $id    = NRG_Secure($id);
    $likes = 0;
...
function NRG_GetMovieCommReplyLikes($id)
{
    global $sqlConnect, $nrg;
    $id    = NRG_Secure($id);
    $likes = 0;
...
function NRG_IsMovieCommentLikeExists($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_IsMovieCommentDisLikeExists($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_IsMovieCommentReplyLikeExists($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_IsMovieCommentReplyDisLikeExists($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_RemoveMovieCommentLikes($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_RemoveMovieCommentDisLikes($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_RemoveMovieCommentReplyDisLikes($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_RemoveMovieCommentReplyLikes($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_AddMovieCommentLikes($id, $movie)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$movie || $movie < 1) {
        return false;
    }
    $user    = $nrg['user']['id'];
    $id      = NRG_Secure($id);
    $movie   = NRG_Secure($movie);
    $comment = NRG_GetMovieCommentData($id);
    $result  = false;
    @NRG_RemoveMovieCommentDisLikes($id);
    if ($comment && !empty($comment) && !NRG_IsMovieCommentLikeExists($id)) {
        $sql   = "INSERT INTO " . T_BM_LIKES . "
    ...
    } else if ($comment && !empty($comment) && NRG_RemoveMovieCommentLikes($id)) {
        $result = true;
    }
    return $result;
}
function NRG_AddMovieCommentDisLikes($id, $movie)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$movie || $movie < 1) {
        return false;
    ...
    $result  = false;
    $user    = $nrg['user']['id'];
    @NRG_RemoveMovieCommentLikes($id);
    if ($comment && !empty($comment) && !NRG_IsMovieCommentDisLikeExists($id)) {
        $sql   = "INSERT INTO " . T_BM_DISLIKES . "
                    (`id`, `blog_comm_id`,`blog_commreply_id`, `movie_comm_id`,`movie_commreply_id`, `user_id`,`movie_id`)
                        VALUES (NULL, '0', '0','$id','0', '$user','$movie')";
        $query = mysqli_query($sqlConnect, $sql);
    ...
    } else if ($comment && !empty($comment) && NRG_RemoveMovieCommentDisLikes($id)) {
        $result = true;
    }
    return $result;
}
function NRG_AddMovieCommReplyLikes($id, $movie)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$movie || $movie < 1) {
        return false;
    ...
    $id      = NRG_Secure($id);
    $movie   = NRG_Secure($movie);
    $comment = NRG_GetMovieCommReplyData($id);
    $result  = false;
    $user    = $nrg['user']['id'];
    if ($comment && !empty($comment) && !NRG_IsMovieCommentReplyLikeExists($id)) {
        $sql   = "INSERT INTO " . T_BM_LIKES . "
                    (`id`, `blog_comm_id`,`blog_commreply_id`, `movie_comm_id`, `movie_commreply_id`, `user_id`,`movie_id`)
        ...
        if ($query) {
            $result = true;
            @NRG_RemoveMovieCommentReplyDisLikes($id);
        }
    } else if ($comment && !empty($comment) && NRG_RemoveMovieCommentReplyLikes($id)) {
        $result = true;
    }
    return $result;
}
function NRG_AddMovieCommReplyDisLikes($id, $movie)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !$movie || $movie < 1) {
        return false;
    ...
    $result  = false;
    $user    = $nrg['user']['id'];
    if ($comment && !empty($comment) && !NRG_IsMovieCommentReplyDisLikeExists($id)) {
        $sql   = "INSERT INTO " . T_BM_DISLIKES . "
                    (`id`, `blog_comm_id`,`blog_commreply_id`, `movie_comm_id`,`movie_commreply_id`, `user_id`,`movie_id`)
                        VALUES (NULL, '0', '0','0','$id', '$user','$movie')";
        $query = mysqli_query($sqlConnect, $sql);
        if ($query) {
            $result = true;
            @NRG_RemoveMovieCommentReplyLikes($id);
        }
    } else if ($comment && !empty($comment) && NRG_RemoveMovieCommentReplyDisLikes($id)) {
        $result = true;
    }
    return $result;
}
function NRG_GetSideBarAds()
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    }
    $user_gender  = $nrg['user']['gender'];
    $user_id      = $nrg['user']['user_id'];
    $user_country = $nrg['user']['country_id'];
    $query_one    = '';
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['ad_media']    = NRG_GetMedia($fetched_data['ad_media']);
            $fetched_data['headline']    = NRG_GetShortTitle($fetched_data['headline'], false, 30);
            $fetched_data['description'] = NRG_GetShortTitle($fetched_data['description'], false, 60);
            if ($fetched_data['bidding'] == 'views') {
                @NRG_RegisterAdConversionView($fetched_data['id']);
            }
            $data[] = $fetched_data;
        }
    }
...
function NRG_DeleteMovieComment($id, $movie)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !NRG_IsMovieCommentOwner($id) || !$movie || $movie < 1) {
        return false;
    }
    $movie = NRG_Secure($movie);
...
function NRG_DeleteMovieCommReply($id, $movie)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1 || !NRG_IsMovieCommReplyOwner($id) || !$movie || $movie < 1) {
        return false;
    }
...
function NRG_UpdateUserAds($id, $update_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || empty($update_data)) {
        return false;
    }
    foreach ($update_data as $field => $data) {
        $update[] = '`' . $field . '` = \'' . $data . '\'';
...
function NRG_IsAdsOwnerNotAdmin($ads_id = 0, $user_id = 0)
{
    global $sqlConnect, $nrg;
    if (empty($user_id)) {
        $user_id = $nrg['user']['user_id'];
    ...
    if (mysqli_num_rows($query)) {
        if (!empty($query)) {
            $fetched_data = mysqli_fetch_assoc($query);
            if ($fetched_data['user_id'] == $nrg['user']['user_id']) {
                $result = true;
            }
        }
    }
...
function NRG_IsAdsOwner($ads_id = 0, $user_id = 0)
{
    global $sqlConnect, $nrg;
    if (empty($user_id)) {
        $user_id = $nrg['user']['user_id'];
    ...
    if (mysqli_num_rows($query)) {
        if (!empty($query)) {
            $fetched_data = mysqli_fetch_assoc($query);
            if ($fetched_data['user_id'] == $nrg['user']['user_id'] || $nrg['user']['admin'] == 1 || NRG_IsModerator() == true) {
                $result = true;
            }
        }
    }
...
function NRG_DeleteUserAd($id = false)
{
    global $sqlConnect, $nrg, $db;
    if ($nrg['loggedin'] == false || !$id || !NRG_IsAdsOwner($id)) {
        return false;
    }
    $ad = $db->where('id', $id)->getOne(T_USER_ADS);
    if (!empty($ad) && !empty($ad->ad_media)) {
        @unlink($ad->ad_media);
        NRG_DeleteFromToS3($ad->ad_media);
    }
    $query_one = "DELETE FROM " . T_USER_ADS . "  WHERE `id` = {$id} ";
    $query     = mysqli_query($sqlConnect, $query_one);
    return $query;
}
function NRG_GetMyAds($args = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $ad = NRG_GetUserAdData($fetched_data['id']);
            if ($ad && !empty($ad)) {
                $ad['name']      = NRG_GetShortTitle($ad['name']);
                $ad['edit-url']  = NRG_SeoLink('index.php?link1=edit-ads&id=' . $ad['id']);
                $ad['chart-url'] = NRG_SeoLink('index.php?link1=chart-ads&id=' . $ad['id']);
                $data[]          = $ad;
            }
        }
    }
...
function NRG_GetMytransactions($args = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            if ($fetched_data['kind'] == 'RECEIVED') {
                $fetched_data['notes'] = str_replace('{text}', $fetched_data['notes'], $nrg['lang']['trans_successfully_received_from']);
            }
            if ($fetched_data['kind'] == 'SENT') {
                $fetched_data['notes'] = str_replace('{text}', $fetched_data['notes'], $nrg['lang']['trans_money_sent_to']);
            }
            if ($fetched_data['kind'] == 'DONATE') {
                $fetched_data['notes'] = str_replace('{text}', $fetched_data['notes'], $nrg['lang']['trans_doanted_to']);
            }
            if ($fetched_data['kind'] == 'PRO') {
                $note = json_decode($fetched_data['notes'], true);
                if (!empty($note)) {
                    $fetched_data['notes'] = $note;
                    if (in_array($fetched_data['notes']['pro_type'], array_keys($nrg['pro_packages']))) {

                        $pro_name = $nrg["pro_packages"][$fetched_data['notes']['pro_type']]['name'];
                        $fetched_data['notes'] = str_replace('{text}', $pro_name . $nrg['lang'][$fetched_data['notes']['method_type']], $nrg['lang']['trans_upgrade_to_pro']);
                    } else {
                        $fetched_data['notes'] = str_replace('{text}', $nrg['lang'][$fetched_data['notes']['method_type']], $nrg['lang']['trans_upgrade_to_pro']);
                    }
                } else {
                    $fetched_data['notes'] = $nrg['lang']['upgrade_to_pro'];
                }
            }
            if ($fetched_data['kind'] == 'PURCHASE') {
                $fetched_data['notes'] = $nrg['lang']['product_purchase'];
            }
            if ($fetched_data['kind'] == 'SALE') {
                $fetched_data['notes'] = $nrg['lang']['product_sale'];
            }
            $data[] = $fetched_data;
        }
    }
...
function NRG_GetUserAdData($id)
{
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data)) {
            $fetched_data['user_data']   = NRG_UserData($fetched_data['user_id']);
            $fetched_data['is_owner']    = NRG_IsAdsOwner($fetched_data['id']);
...
function NRG_GetPostAds($last_id = 0)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    $query_one    = '';
    $con_list     = false;
    if (!empty($nrg['ad-con']) && !empty($nrg['ad-con']['ads'])) {
        $con_list = implode(',', $nrg['ad-con']['ads']);
    }
...
function NRG_GetAdsByType($type = 'post', $last_id = 0)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_GetAds($last_id = 0)
{
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            $fetched_data['is_owner']  = NRG_IsAdsOwner($fetched_data['id']);
            $fetched_data['ad_media']  = NRG_GetMedia($fetched_data['ad_media']);
            $fetched_data['edit-url']  = NRG_SeoLink('index.php?link1=manage-ads&id=' . $fetched_data['id']);
            $data[]                    = $fetched_data;
        }
    }
    return $data;
}
function NRG_GetAllStories($last_id = 0)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_IsConversionExists($id)
{
    global $nrg;
    $adv_ids  = $nrg['ad-con'];
    $result   = false;
    $is_admin = NRG_IsAdsOwnerNotAdmin($id);
...
function NRG_RegisterAdConversionClick($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id) {
        return false;
    }
    $ad       = NRG_GetUserAdData($id);
    $user     = $nrg['user']['user_id'];
    $result   = false;
    $is_admin = NRG_IsAdsOwnerNotAdmin($id);
    if ($ad && is_array($ad) && !empty($ad) && isset($ad['user_data']) && !NRG_IsConversionExists($id) && $is_admin == false) {
        $price = $nrg['config']['ad_c_price'];
        if ($ad['appears'] == 'entire') {
            $price = $nrg['config']['ad_c_price'] * 1.5;
        }
...
function NRG_RegisterAdConversionView($id)
{
    ...
    if ($ad && is_array($ad) && !empty($ad) && isset($ad['user_data']) && !NRG_IsConversionExists($id) && $is_admin == false) {
        $ad_user_id = $ad['user_data']['user_id'];
        $price      = $nrg['config']['ad_v_price'];
        if ($ad['appears'] == 'entire') {
            $price = $nrg['config']['ad_v_price'] * 1.5;
        }
        $wallet = $ad['user_data']['wallet'] -= $price;
        $result = mysqli_query($sqlConnect, "UPDATE " . T_USERS . " SET `wallet` = '$wallet' WHERE `user_id` = '$ad_user_id'");
        NRG_RegisterAdView($id);
        cache($ad_user_id, 'users', 'delete');
    } else if ($ad && is_array($ad) && !empty($ad) && isset($ad['user_data']) && $is_admin == false) {
        $result = NRG_RegisterAdView($id);
    } else {
        return true;
    }
    return $result;
}
function NRG_RegisterAdClick($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id) {
        return false;
    ...
    if ($ad && is_array($ad) && !empty($ad)) {
        ...
        $result  = NRG_UpdateUserAds($id, array(
            'clicks' => ($ad['clicks'] + 1)
        ));
    }
    return $result;
}
function NRG_RegisterAdView($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id) {
        return false;
...
function NRG_IsStoryOwner($id)
{
    ...
    if (mysqli_num_rows($query)) {
        $fetched_data = mysqli_fetch_assoc($query);
        if ($fetched_data['user_id'] == $user || NRG_IsAdmin() || NRG_IsModerator()) {
            $result = true;
        }
    }
    return $result;
}
function NRG_InsertUserStory($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_InsertUserStoryMedia($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_GetStroies($args = array())
{
    global $sqlConnect, $nrg, $db;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $story_images = NRG_GetStoryMedia($fetched_data['id'], 'image');
            if (count($story_images) > 0) {
                $fetched_data['thumb']  = array_shift($story_images);
                $fetched_data['images'] = $story_images;
            }
            $fetched_data['user_data'] = NRG_UserData($fetched_data['user_id']);
            $fetched_data['videos']    = NRG_GetStoryMedia($fetched_data['id'], 'video');
            $fetched_data['is_owner']  = ($fetched_data['user_id'] == $nrg['user']['id'] || NRG_IsAdmin() || NRG_IsModerator()) ? true : false;
            $fetched_data['reaction']  = NRG_GetPostReactionsTypes($fetched_data['id'], 'story');
            $fetched_data['is_viewed']  = $db->where('story_id', $fetched_data['id'])->where('user_id', $nrg['user']['id'])->getValue(T_STORY_SEEN, 'COUNT(*)');
            $data[]                    = $fetched_data;
        }
    }
    return $data;
}
function NRG_GetStoryMedia($id = false, $type = 'image')
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['filename'] = NRG_GetMedia($fetched_data['filename']);
            $data[]                   = $fetched_data;
        }
    }
    return $data;
}
function NRG_CountUserStatus($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_DeleteStatus($id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !NRG_IsStoryOwner($id)) {
        return false;
    }
    $id         = NRG_Secure($id);
    $data       = array();
...
function NRG_UpdateCommentReply($id, $update_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || empty($update_data) || !$id || !is_numeric($id) || $id < 1) {
        return false;
    }
...
function NRG_GetUsersByName($name = '', $friends = false, $limit = 25)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$name) {
        return false;
    }
    $user        = $nrg['user']['id'];
    $name        = NRG_Secure($name);
    $data        = array();
    $sub_sql     = "";
    $t_users     = T_USERS;
...
function NRG_GetUserIds()
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !NRG_IsAdmin()) {
        return false;
    }
    $data  = array();
    $admin = $nrg['user']['id'];
...
function NRG_RegisterAdminNotification($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !NRG_IsAdmin() || empty($registration_data) || empty($registration_data['text'])) {
        return false;
    }
...
function NRG_HidePost($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || $id < 1) {
        return false;
    }
...
function NRG_SendVerificationRequest($registration_data = array())
{
    global $sqlConnect, $nrg, $db;
    if ($nrg['loggedin'] == false || !is_array($registration_data) || empty($registration_data) || NRG_IsAdmin()) {
        return false;
    }
    $notification_data_array = array(
        'recipient_id' => 0,
...
function NRG_UpdateVerificationRequest($id = false, $update_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_array($update_data) || empty($update_data) || !$id) {
        return false;
    }
    $id = NRG_Secure($id);
    foreach ($update_data as $field => $data) {
...
function NRG_IsVerificationRequestExists()
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_IsThisPostShared($id = 0)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_IsPostShared($id = 0)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_IsSharedPostExists($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id) || $id < 1) {
        return false;
...
function NRG_SharePost($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || $id < 1) {
        return false;
    ...
    $time = time();
    $sql  = '';
    if (NRG_IsPostShared($id)) {
        $shared_post = NRG_PostData($id);
    ...
    if ($post) {
        ...
        $post_data['post_url']    = NRG_SeoLink('index.php?link1=post&id=' . $id);
...
function NRG_GenirateSiteMap($updating = 'daily')
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !NRG_IsAdmin()) {
        return false;
...
function NRG_GetAdminInvitation()
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !NRG_IsAdmin()) {
        return false;
    ...
    $data  = array();
    $site  = $nrg['config']['site_url'] . '/register?invite=';
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function NRG_InsertAdminInvitation()
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !NRG_IsAdmin()) {
        return false;
...
function NRG_DeleteAdminInvitation($col = '', $val = false)
{
    global $sqlConnect, $nrg;
    if (!$val && !$col) {
        return false;
...
function NRG_IsAdminInvitationExists($code = false)
{
    global $sqlConnect, $nrg;
    if (!$code) {
        return false;
...
function NRG_FriendPrivacy($user_id = false, $friend_privacy = false)
{
    global $sqlConnect, $nrg;
    if (!$user_id) {
        return false;
    ...
    if ($nrg['loggedin'] == true) {
        ...
        $loggedin_user  = $nrg['user']['id'];
        $user_id        = NRG_Secure($user_id);
        $friend_privacy = NRG_Secure($friend_privacy);
        $privacy        = false;
        if ($friend_privacy == 0) {
            return true;
...
function NRG_IsGroupUserExists($user_id = false, $group_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_numeric($user_id) || !is_numeric($group_id)) {
        return false;
    }
...
function NRG_IsPageAdminExists($user_id = false, $page_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_numeric($user_id) || !is_numeric($page_id)) {
        return false;
    }
...
function NRG_AddGroupAdmin($user_id = false, $group_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_numeric($user_id) || !is_numeric($group_id)) {
        return false;
    ...
    if ($nrg['user']['id'] != $group['user_id'] && !NRG_IsGroupUserExists($nrg['user']['id'], $group_id)) {
        return false;
    }
    if (NRG_IsGroupUserExists($user_id, $group_id)) {
...
function NRG_CheckGroupAdminPassword($password = false, $group_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_numeric($password) || !is_numeric($group_id)) {
        return false;
    }
    $user_id  = NRG_Secure($nrg['user']['user_id']);
    $group_id = NRG_Secure($group_id);
    ...
    if (NRG_IsGroupUserExists($user_id, $group_id)) {
        ...
        if (mysqli_num_rows($data) == 1) {
            $fetched_data = mysqli_fetch_assoc($data);
            if (NRG_HashPassword($password, $fetched_data['password'])) {
                $match = true;
            }
        }
    }
...
function NRG_CheckPageAdminPassword($password = false, $page_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_numeric($page_id)) {
        return false;
    }
    $user_id = NRG_Secure($nrg['user']['user_id']);
    $page_id = NRG_Secure($page_id);
    $match   = false;
    if (NRG_IsPageAdminExists($user_id, $page_id)) {
        $sql  = "SELECT `password` FROM " . T_USERS . " WHERE `user_id` = {$user_id}";
        $data = mysqli_query($sqlConnect, $sql);
        if (mysqli_num_rows($data) == 1) {
            $fetched_data = mysqli_fetch_assoc($data);
...
function NRG_AddPageAdmin($user_id = false, $page_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_numeric($user_id) || !is_numeric($page_id)) {
        return false;
    ...
    if ($nrg['user']['id'] != $page['user_id'] && !NRG_IsPageAdminExists($nrg['user']['id'], $page_id) && !NRG_IsAdmin() && !NRG_IsModerator()) {
        return false;
    }
    if (NRG_IsPageAdminExists($user_id, $page_id)) {
...
function NRG_GetPageAdmins($page_id = false, $return = 'all')
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_numeric($page_id)) {
        return false;
...
function NRG_GetPageAdminInfo($user_id, $page_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || empty($page_id) || !is_numeric($page_id) || empty($user_id) || !is_numeric($user_id)) {
        return false;
    }
    $page_id = NRG_Secure($page_id);
    $user_id = NRG_Secure($user_id);
...
function NRG_CreateGChat($name = false, $parts = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_array($parts) || !$name || count($parts) < 1) {
        return false;
    }
    $user  = NRG_Secure($nrg['user']['id']);
    $time  = time();
...
function NRG_UpdateGChat($id = false, $update_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_array($update_data) || empty($update_data) || !$id) {
        return false;
...
function NRG_GroupTabData($id = false, $update_seen = true)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id)) {
        return false;
    ...
    $query = mysqli_query($sqlConnect, $sql);
    if ($update_seen == true) {
        @NRG_UpdateGChatLastSeen($id);
    }
    if ($query && mysqli_num_rows($query) > 0) {
        $data             = mysqli_fetch_assoc($query);
        $data['avatar']   = NRG_GetMedia($data['avatar']);
        $data['messages'] = NRG_GetGroupMessages(array(
            'group_id' => $data['group_id']
        ));
    }
    return $data;
}
function NRG_UpdateGChatLastSeen($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id)) {
        return false;
...
function NRG_IsGChatOwner($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id)) {
        return false;
    }
    $group = NRG_GroupTabData($id);
    $owner = false;
    if (is_array($group) && isset($group['user_id'])) {
        $group_admin = $group['user_id'];
        $user_id     = $nrg['user']['id'];
        $owner       = ($group_admin == $user_id || NRG_IsAdmin()) ? true : false;
    }
    return $owner;
}
function NRG_GetChatGroupLastMessage($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !is_numeric($id)) {
        return false;
...
function NRG_GetChatGroups($after_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    $sql   = "SELECT * FROM " . T_GROUP_CHAT . "
                WHERE (`user_id` = {$user} OR `group_id` IN
                   (SELECT `group_id` FROM NRG_GroupChatUsers  WHERE `user_id` = {$user} AND active = 1)) {$sub_sql} ORDER BY `time` DESC";
    $query = mysqli_query($sqlConnect, $sql);
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['user_data']    = NRG_UserData($fetched_data['user_id']);
...
// group data
function NRG_GetChatGroupData($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || empty($id) || !is_numeric($id) || $id < 1) {
        return false;
    }
    $user  = NRG_Secure($nrg['user']['id']);
    $id    = NRG_Secure($id);
    ...
    $sql   = "SELECT * FROM " . T_GROUP_CHAT . "
                WHERE (`user_id` = {$user} OR `group_id` IN
                   (SELECT `group_id` FROM NRG_GroupChatUsers  WHERE `user_id` = {$user})) AND `group_id` = {$id}";
    $query = mysqli_query($sqlConnect, $sql);
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['user_data']    = NRG_UserData($fetched_data['user_id']);
...
// group data
function NRG_CheckLastGroupAction()
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $last_message = NRG_GetChatGroupLastMessage($fetched_data['group_id']);
...
function NRG_CheckLastGroupUnread()
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $last_message = NRG_GetChatGroupLastMessage($fetched_data['group_id']);
            if (!empty($last_message) && isset($last_message['time']) && isset($last_message['from_id']) && $last_message['from_id'] != $nrg['user']['id']) {
...
function NRG_GetGChatMemebers($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_IsGChatMemebers($group_id)
{
    global $sqlConnect, $nrg, $db;
    if ($nrg['loggedin'] == false || empty($group_id)) {
        return false;
    }
    $id      = NRG_Secure($group_id);
    $user_id = $nrg['user']['id'];
...
function NRG_CountGroupChatRequests()
{
    global $sqlConnect, $nrg, $db;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_ClearGChat($group_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !NRG_IsGChatOwner($group_id)) {
        return false;
    }
    return mysqli_query($sqlConnect, "DELETE FROM " . T_MESSAGES . " WHERE `group_id` = {$group_id}");
}
function NRG_DeleteGChat($group_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !NRG_IsGChatOwner($group_id)) {
        return false;
...
function NRG_ExitGChat($group_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$group_id || $group_id < 1) {
        return false;
    }
    $user = NRG_Secure($nrg['user']['id']);
...
function NRG_IsGChatMemeberExists($group_id = false, $user_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$group_id || $group_id < 1) {
        return false;
    }
    $user  = NRG_Secure($user_id);
    $group = NRG_Secure($group_id);
    if ($user_id === false) {
        $user = $nrg['user']['user_id'];
    }
...
function NRG_AddGChatPart($group_id = false, $user_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$group_id || !$user_id) {
        return false;
    }
    $group = NRG_Secure($group_id);
    $user  = NRG_Secure($user_id);
    $code  = 0;
    if (!NRG_IsGChatOwner($group)) {
        return false;
    }
    if (NRG_IsGChatMemeberExists($group, $user)) {
...
function NRG_ReportUser($user = false, $text = '', $reason = '')
{
    global $sqlConnect, $nrg, $db;
    if ($nrg['loggedin'] == false || !$user) {
        return false;
    }
    if ($user == $nrg['user']['id']) {
        return false;
    ...
    if (!NRG_IsReportExists($user, 'user')) {
...
function NRG_ReportPage($page = false, $text = '')
{
    global $sqlConnect, $nrg, $db;
    if ($nrg['loggedin'] == false || !$page) {
        return false;
    }
    $user      = $nrg['user']['id'];
    $page_data = NRG_PageData($page);
    if (!is_array($page_data) || !isset($page_data['user_id'])) {
        return false;
    }
    if ($page_data['user_id'] == $nrg['user']['id']) {
        return false;
    }
    $user_id = $nrg['user']['id'];
    $time    = time();
    $code    = null;
    if (!NRG_IsReportExists($page, 'page')) {
...
function NRG_ReportGroup($group = false, $text = '')
{
    global $sqlConnect, $nrg, $db;
    if ($nrg['loggedin'] == false || !$group) {
        return false;
    }
    $user       = $nrg['user']['id'];
    $group_data = NRG_GroupData($group);
    if (!is_array($group_data) || !isset($group_data['user_id'])) {
        return false;
    }
    if ($group_data['user_id'] == $nrg['user']['id']) {
        return false;
    }
    $user_id = $nrg['user']['id'];
    $time    = time();
    $code    = null;
    if (!NRG_IsReportExists($group, 'group')) {
...
function NRG_IsPageRatingExists($page_id = false, $user_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_numeric($user_id) || !is_numeric($page_id)) {
        return false;
...
function NRG_RatePage($page_id = false, $value = false, $text = '')
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$page_id || !$value) {
        return false;
    }
    ...
    $user    = $nrg['user']['id'];
    $rate    = false;
    if (NRG_IsPageRatingExists($page_id, $user)) {
        return false;
    }
    return mysqli_query($sqlConnect, "INSERT INTO " . T_PAGE_RATING . " (`user_id`,`page_id`,`valuation`,`review`) VALUES ('$user','$page_id','$value','$text')");
}
function NRG_PageRating($page_id = false, $user_id = 0)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$page_id || !is_numeric($page_id)) {
        return false;
    }
...
function NRG_GetPageReviews($page_id = false, $after_id = false, $limit = 10)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$page_id || !is_numeric($page_id)) {
        return false;
...
function NRG_RegisterFamilyMember($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_array($registration_data) || empty($registration_data)) {
        return false;
    }
    $fields = '`' . implode('`, `', array_keys($registration_data)) . '`';
    $data   = '\'' . implode('\', \'', $registration_data) . '\'';
...
function NRG_DeleteFamilyMember($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id) {
        return false;
...
function NRG_AcceptFamilyMember($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id) {
        return false;
...
function NRG_AcceptRelationRequest($id = false, $member = fasle, $type = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || !$member || !$type) {
        return false;
    }
    $user = $nrg['user']['id'];
...
function NRG_DeleteMyRelationShip()
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_DeleteRelationRequest($id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id) {
        return false;
...
function NRG_IsRelationRequestExists($from_id = false, $to_id = false, $type = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$from_id || !$to_id || !$type) {
        return false;
    }
    $to_id   = NRG_Secure($to_id);
    $from_id = NRG_Secure($from_id);
...
function NRG_IsFamilyMemberExists($user_id = false, $member_id = false, $active = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_numeric($user_id) || !is_numeric($member_id)) {
        return false;
    }
    $sub_sql = '';
...
function NRG_GetFamalyMember($member_id = false, $user_id = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_numeric($user_id) || !is_numeric($member_id)) {
        return false;
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        if ($fetched_data['active'] == 1) {
            $status = $nrg['lang'][$nrg['family'][$fetched_data['member']]];
        } else {
            $pendind = $nrg['lang']['pending'];
            $status  = $nrg['lang'][$nrg['family'][$fetched_data['member']]] . " ($pendind) ";
        }
    }
    return $status;
}
function NRG_GetFamaly($user_id = false, $after_id = false, $active = 1, $requests = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || ($active != 1 && $active != 0)) {
        return false;
    }
    $user = NRG_Secure($user_id);
    $data = array();
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            $fetched_data['user_data'] = NRG_UserData(($user_data));
...
function NRG_RegisterRelationship($registration_data = array())
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !is_array($registration_data) || empty($registration_data)) {
        return false;
...
function NRG_UserRelationship($user = false)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        $with_data    = NRG_UserData($with_id);
        if ($fetched_data['relationship'] == 1) {
            $relation = $nrg['relationship'][1];
        }
        if ($fetched_data['relationship'] == 2) {
            $relation = $nrg['lang']['relation_with'] . ' <a href="' . $with_data['url'] . '" data-ajax="?link1=timeline&u=' . $with_data['username'] . '">@' . $with_data['name'] . '</a>';
        } else if ($fetched_data['relationship'] == 3) {
            $relation = $nrg['lang']['married_to'] . ' <a href="' . $with_data['url'] . '" data-ajax="?link1=timeline&u=' . $with_data['username'] . '">@' . $with_data['name'] . '</a>';
        } else if ($fetched_data['relationship'] == 4) {
            $relation = $nrg['lang']['engaged_to'] . ' <a href="' . $with_data['url'] . '" data-ajax="?link1=timeline&u=' . $with_data['username'] . '">@' . $with_data['name'] . '</a>';
        }
    } else if (array_key_exists($user_data['relationship_id'], $nrg['relationship'])) {
        $relation = $nrg['relationship'][$user_data['relationship_id']];
    }
    return $relation;
}
...
{
    global $nrg;
    if ($nrg['loggedin'] == false || !$birthday || $birthday < 1) {
        return false;
    }
    $user_age = '';
    try {
        ...
        $years        = $age->y;
        if ($years) {
            $user_age = $age->y . ' ' . $nrg['lang']['years_old'];
        }
    } catch (Exception $e) {
    }
    return $user_age;
}
function NRG_GetUserCountryName($user_data = array())
{
    global $nrg;
    if ($nrg['loggedin'] == false || empty($user_data)) {
        return false;
    }
    $age       = GetUserAge($user_data['birthday']);
    $user_from = (!empty($age)) ? $age : '';
    if ($user_data['country_id'] > 0 && in_array($user_data['country_id'], array_keys($nrg['countries_name']))) {
        $user_from_arr = array(
            0 => $user_from,
            1 => $user_data['address'],
            2 => $nrg['countries_name'][$user_data['country_id']]
        );
        $data          = array();
...
function NRG_GetNearbyUsers($args = array())
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false || empty($args)) {
        return false;
    }
    $options      = array(
        "offset" => false,
    ...
    $args         = array_merge($options, $args);
    $offset       = NRG_Secure($args['offset']);
    $gender       = NRG_Secure($args['gender']);
    $name         = NRG_Secure($args['name']);
    $loc_distance = NRG_Secure($args['distance']);
    $status       = NRG_Secure($args['status']);
    $relship      = NRG_Secure($args['relship']);
    $limit        = NRG_Secure($args['limit']);
    $unit         = 6371;
    $user_lat     = $nrg['user']['lat'];
    $user_lng     = $nrg['user']['lng'];
    ...
    if ($relship && in_array($relship, array_keys($nrg['relationship']))) {
        $sub_sql .= " AND `relationship_id`  = '$relship' ";
    }
    if ($offset && is_numeric($offset) && $offset > 0) {
        $sub_sql .= " AND `user_id` <  '$offset' AND `user_id` <> '$offset' ";
    }
    if ($gender && in_array($gender, array_keys($nrg['genders']))) {
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['user_data']        = NRG_UserData($fetched_data['user_id']);
            $fetched_data['user_data']['age'] = NRG_GetUserCountryName($fetched_data['user_data']);
...
function NRG_GetNearbyUsersCount($args = array())
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false || empty($args)) {
        return false;
...
function NRG_CountStories($user_id = 0)
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_GetFriendsStatus($data_array = array('limit' => 8, 'user_id' => 0, 'offset' => 0))
{
    global $nrg, $sqlConnect, $db;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    if (empty($data_array['user_id'])) {
        $data_array['user_id'] = $nrg['user']['user_id'];
    }
    $data     = array();
    $user_id  = NRG_Secure($data_array['user_id']);
    ...
    if (!empty($data_array['offset'])) {
        $offset       = NRG_Secure($data_array['offset']);
    ...
    while ($fetched_data = mysqli_fetch_assoc($query_run)) {
        ...
        $not_seen = $db->where("(user_id = " . $fetched_data['user_id'] . " AND id NOT IN (SELECT story_id FROM " . T_STORY_SEEN . " WHERE user_id = " . $nrg['user']['id'] . "))")->getValue(T_USER_STORY, 'COUNT(*)');
        $fetched_data['have_not_seen'] = 0;
        if ($not_seen > 0 && $fetched_data['user_id'] != $nrg['user']['id']) {
        ...
        $fetched_data['thumb']['filename'] = NRG_GetMedia($fetched_data['thumb']['filename']);
        $fetched_data['videos']            = NRG_GetStoryMedia($fetched_data['id'], 'video');
        $fetched_data['is_owner']          = ($fetched_data['user_id'] == $nrg['user']['id'] || NRG_IsAdmin() || NRG_IsModerator()) ? true : false;
        $data[]                            = $fetched_data;
    }
...
function NRG_GetFriendsStatusAPI($data_array = array('limit' => 8, 'user_id' => 0, 'offset' => 0))
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_HashPassword($password = '', $hashed_password = '')
{
    global $nrg, $sqlConnect;
    if (empty($password)) {
        return '';
...
function NRG_UpdateUserDetails($user_id = 0, $me = false, $time = true, $get_data = false, $counts = false)
{
    global $nrg, $sqlConnect;
    if (empty($user_id)) {
        return false;
    ...
    if ($me == true && is_array($user_id)) {
        $nrg['user_profile'] = $user_id;
    } else {
        $user_id            = NRG_Secure($user_id);
        $nrg['user_profile'] = NRG_UserData($user_id);
    }
    if (empty($nrg['user_profile'])) {
        return false;
    }
    $last_data_update_time = time() - $nrg['config']['update_user_profile'];
    $time_now              = time();
    $cache                 = false;
    if ($time == true) {
        if ($nrg['user_profile']['last_data_update'] < $last_data_update_time) {
            $cache = true;
        }
    } else {
        $cache = true;
    ...
    if ($cache == true) {
        $final_data = array(
            'post_count' => NRG_CountUserPosts($nrg['user_profile']['user_id']),
            'album_count' => NRG_CountUserAlbums($nrg['user_profile']['user_id']),
            'following_count' => NRG_CountFollowing($nrg['user_profile']['user_id']),
            'followers_count' => NRG_CountFollowers($nrg['user_profile']['user_id']),
            'groups_count' => NRG_CountUserGroups($nrg['user_profile']['user_id']),
            'likes_count' => NRG_CountUserLikes($nrg['user_profile']['user_id']),
            'mutual_friends_count' => NRG_CountMutualFriends($nrg['user_profile']['user_id'])
        );
        if ($counts == false) {
            $get_following_ids = NRG_GetFollowing($nrg['user_profile']['user_id'], 'profile', 9);
            $following_ids     = array();
            foreach ($get_following_ids as $key => $user) {
                $following_ids[] = $user['user_id'];
            }
            $get_followers_ids = NRG_GetFollowers($nrg['user_profile']['user_id'], 'profile', 9);
            $followers_ids     = array();
            foreach ($get_followers_ids as $key => $user) {
                $followers_ids[] = $user['user_id'];
            }
            $get_mutual_ids = NRG_GetMutualFriends($nrg['user_profile']['user_id'], 'profile', 9);
            $mutual_ids     = array();
            ...
            $get_likes_ids = NRG_GetLikes($nrg['user_profile']['user_id'], 'profile', 9);
            $likes_ids     = array();
            foreach ($get_likes_ids as $key => $page) {
                $likes_ids[] = $page['page_id'];
            }
            $get_groups_ids = NRG_GetUsersGroups($nrg['user_profile']['user_id'], 9);
            $groups_ids     = array();
        ...
        $user_id = $nrg['user_profile']['user_id'];
        ...
        if ($query && $get_data == true) {
            return NRG_UserData($nrg['user_profile']['user_id']);
        }
    }
}
function NRG_DeleteAllData($type = 0)
{
    global $nrg, $sqlConnect;
    if (empty($type) || NRG_IsAdmin() == false) {
        return false;
    }
    $me = $nrg['user']['user_id'];
    if ($type == 1) {
        ...
        if (mysqli_num_rows($query)) {
            while ($row = mysqli_fetch_assoc($query)) {
                $delete_user = NRG_DeleteUser($row['user_id']);
            }
        }
    }
    if ($type == 2) {
    ...
    if ($type == 5) {
        ...
        if (mysqli_num_rows($query)) {
            while ($row = mysqli_fetch_assoc($query)) {
                $delete_post = NRG_DeletePost($row['id']);
            }
        }
    }
    if ($type == 6) {
...
function NRG_AutoFollow($user_id = 0)
{
    global $nrg, $db;
    if (empty($user_id)) {
        return false;
    ...
    $nrg['loggedin']        = true;
    $nrg['user']['user_id'] = $user_id;
    $get_users             = explode(',', $nrg['config']['auto_friend_users']);
    if (!empty($get_users)) {
        foreach ($get_users as $key => $user) {
            $user      = trim($user);
            $user      = NRG_Secure($user);
            $getUserID = NRG_UserIdFromUsername($user);
            if (!empty($getUserID)) {
                if ($nrg['config']['connectivitySystem'] == 1) {
                    $registerFollow = NRG_RegisterFollow($nrg['user']['user_id'], $getUserID);
                    $registerFollow = NRG_AcceptFollowRequest($getUserID, $nrg['user']['user_id']);
                } else {
                    $registerFollow = NRG_RegisterFollow($getUserID, $nrg['user']['user_id']);
                }
            }
        }
        return true;
...
function NRG_AutoPageLike($user_id = 0)
{
    global $nrg, $db;
    if (empty($user_id)) {
        return false;
    ...
    $nrg['loggedin']        = true;
    $nrg['user']['user_id'] = $user_id;
    $pages_name            = explode(',', $nrg['config']['auto_page_like']);
    if (!empty($pages_name)) {
        foreach ($pages_name as $key => $page_name) {
            $page_name = trim($page_name);
            $page_name = NRG_Secure($page_name);
            $page_id   = NRG_PageIdFromPagename($page_name);
            NRG_RegisterPageLike($page_id, $nrg['user']['user_id']);
        }
        return true;
    } else {
        return false;
...
function NRG_AutoGroupJoin($user_id = 0)
{
    global $nrg, $db;
    if (empty($user_id)) {
        return false;
    ...
    $nrg['loggedin']        = true;
    $nrg['user']['user_id'] = $user_id;
    $groups_name           = explode(',', $nrg['config']['auto_group_join']);
    if (!empty($groups_name)) {
        foreach ($groups_name as $key => $group_name) {
            $group_name = trim($group_name);
            $group_name = NRG_Secure($group_name);
            $group_id   = NRG_GroupIdFromGroupname($group_name);
            NRG_RegisterGroupJoin($group_id, $nrg['user']['user_id']);
        }
        return true;
    } else {
        return false;
...
function NRG_MarkAllChatsAsRead($user_id = 0)
{
    global $nrg, $db;
    if (NRG_IsAdmin() === false) {
...
function NRG_TwoFactor($username = '', $id_or_u = 'user')
{
    global $nrg, $db;
    if (empty($username)) {
        return true;
    }
    if ($nrg['config']['two_factor'] == 0) {
        return true;
    }
    if ($id_or_u == 'id') {
        $getuser = NRG_UserData($username);
    } else {
        $getuser = NRG_UserData(NRG_UserIdForLogin($username));
    }
    ...
    if (!empty($getuser['phone_number']) && ($nrg['config']['two_factor_type'] == 'both' || $nrg['config']['two_factor_type'] == 'phone')) {
        $send_message = NRG_SendSMSMessage($getuser['phone_number'], $message);
    }
    if ($nrg['config']['two_factor_type'] == 'both' || $nrg['config']['two_factor_type'] == 'email') {
...
function NRG_VerfiyIP($username = '')
{
    global $nrg, $db;
    if (empty($username)) {
        return false;
    }
    if ($nrg['config']['login_auth'] == 0) {
        return true;
    }
    $getuser   = NRG_UserData(NRG_UserIdForLogin($username));
    ...
    if ($getIpInfo['status'] == 'success' && !empty($getIpInfo['regionName']) && !empty($getIpInfo['countryCode']) && !empty($getIpInfo['timezone']) && !empty($getIpInfo['city'])) {
        ...
        if (empty($getuser['last_login_data'])) {
            ...
            if (($getIpInfo['regionName'] != $lastLoginData['regionName']) || ($getIpInfo['countryCode'] != $lastLoginData['countryCode']) || ($getIpInfo['timezone'] != $lastLoginData['timezone']) || ($getIpInfo['city'] != $lastLoginData['city'])) {
                ...
                $code                       = rand(111111, 999999);
                $hash_code                  = md5($code);
                $nrg['email']['username']    = $getuser['name'];
                $nrg['email']['countryCode'] = $getIpInfo['countryCode'];
                $nrg['email']['timezone']    = $getIpInfo['timezone'];
                $nrg['email']['email']       = $getuser['email'];
                $nrg['email']['ip_address']  = $get_ip;
                $nrg['email']['code']        = $code;
                $nrg['email']['city']        = $getIpInfo['city'];
                $nrg['email']['date']        = date("Y-m-d h:i:sa");
                $update_code                = $db->where('user_id', $getuser['user_id'])->update(T_USERS, array(
                    'email_code' => $hash_code
                ));
                cache($getuser['user_id'], 'users', 'delete');
                $email_body                 = NRG_LoadPage("emails/unusual-login");
                $send_message_data          = array(
                    'from_email' => $nrg['config']['siteEmail'],
                    'from_name' => $nrg['config']['siteName'],
                    'to_email' => $getuser['email'],
...
function NRG_GetAllStatus()
{
    global $nrg, $db;
    $user_id = $nrg['user']['user_id'];
    return $db->rawQuery("SELECT DISTINCT user_id,title,description,posted,expire,thumbnail,(SELECT MAX(us.id) FROM " . T_USER_STORY . " us WHERE us.user_id = " . T_USER_STORY . ".user_id) AS id  FROM " . T_USER_STORY . " WHERE (user_id IN (SELECT following_id FROM " . T_FOLLOWERS . " WHERE follower_id = '$user_id') OR user_id = $user_id) AND user_id IN (SELECT user_id FROM " . T_USERS . " WHERE active = '1') GROUP BY user_id ORDER BY id DESC");
}
function NRG_SharePostOn($id = false, $type_id = 0, $type = '')
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || !$id || $id < 1 || !$type_id) {
        return false;
    }
    $id        = NRG_Secure($id);
    $user      = $nrg['user']['id'];
...
function NRG_updateProInfo($update_data)
{
    global $sqlConnect, $nrg;
...
// manage packages
function NRG_DeleteAllUserPosts($user_id)
{
    global $sqlConnect, $nrg;
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 1 || !NRG_IsAdmin()) {
        return false;
    }
    $posts = mysqli_query($sqlConnect, "SELECT `id` FROM " . T_POSTS . " WHERE `user_id` = {$user_id}");
    if ($posts) {
        while ($fetched_data = mysqli_fetch_assoc($posts)) {
            NRG_DeletePost($fetched_data['id']);
        }
        return true;
    }
    return false;
}
function NRG_InsertBankTrnsfer($inserted_data)
{
    global $nrg, $sqlConnect, $db;
...
function NRG_GetPageJobs($page_id)
{
    global $nrg, $sqlConnect, $db;
    if ($nrg['loggedin'] == false || empty($page_id)) {
        return false;
    }
    $data    = array();
    $page_id = NRG_Secure($page_id);
    ...
    if (!empty($jobs)) {
        foreach ($jobs as $key => $value) {
            ...
            $apply               = $db->where('user_id', $nrg['user']['id'])->where('job_id', $data[$key]['id'])->getValue(T_JOB_APPLY, 'COUNT(*)');
            $data[$key]['apply'] = ($apply > 0) ? true : false;
            // $post = $db->where('job_id',$data[$key]['id'])->getOne(T_POSTS);
            // $data[$key]['story'] = array();
            // if (!empty($post)) {
            //     $data[$key]['story'] = NRG_PostData($post->id);
            // }
            $data[$key]['page']  = $page;
        }
    }
...
function NRG_GetJobById($job_id)
{
    global $nrg, $sqlConnect, $db;
    if (empty($job_id)) {
        return false;
    ...
    if (mysqli_num_rows($sql)) {
        ...
        if (!empty($jobs)) {
            ...
            if (!empty($jobs['page_id'])) {
                ...
                $jobs['user'] = NRG_UserData($jobs['user_id']);
            }
            $jobs['apply'] = false;
            if ($nrg['loggedin']) {
                $user_id       = $nrg['user']['id'];
            ...
            $sql                 = mysqli_query($sqlConnect, $query_one);
            $fetched_data        = mysqli_fetch_assoc($sql);
            $jobs['url']         = NRG_SeoLink('index.php?link1=post&id=' . $fetched_data['id']);
...
function NRG_GetAllJobs($filter_data = array())
{
    global $nrg, $sqlConnect;
    $data      = array();
    ...
    if (!empty($filter_data['type'])) {
        $type = NRG_Secure($filter_data['type']);
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $job    = NRG_GetJobById($fetched_data['id']);
            //$products['seller'] = NRG_UserData($fetched_data['user_id']);
            $data[] = $job;
        }
    }
...
function NRG_GetApplyJob($info = array())
{
    ...
    if (!empty($info['user_id']) && is_numeric($info['user_id']) && $info['user_id'] > 0) {
        $user_id = NRG_Secure($info['user_id']);
        if (!empty($info['offset']) && is_numeric($info['offset']) && $info['offset'] > 0) {
            $offset = NRG_Secure($info['offset']);
            $db->where('id', $offset, '>');
        }
    ...
    if (!empty($jobs)) {
        foreach ($jobs as $key => $value) {
            $data[$key]              = (array) $value;
            $data[$key]['job_info']  = NRG_GetJobById($value->job_id);
            $data[$key]['user_data'] = NRG_UserData($value->user_id);
        }
        // $jobs['page'] = $page;
        // $apply = $db->where('user_id',$nrg['user']['id'])->where('job_id',$job_id)->getValue(T_JOB_APPLY,'COUNT(*)');
        // $jobs['apply'] = ($apply > 0) ? true : false;
        // $post = $db->where('job_id',$job_id)->getOne(T_POSTS);
        // $jobs['url'] = NRG_SeoLink('index.php?link1=post&id=' . $post->id);
        // $jobs['apply_count'] = $apply;
    }
    return $data;
}
function NRG_GetCommonUsers($args = array())
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false) {
        return false;
    ...
    $args        = array_merge($options, $args);
    $before      = NRG_Secure($args['before']);
    $after       = NRG_Secure($args['after']);
    $limit       = NRG_Secure($args['limit']);
    $user        = $nrg['user']['id'];
    $t_users     = T_USERS;
    $t_followers = T_FOLLOWERS;
    ...
    $data        = array();
    $sub_sql     = "";
    if (!empty($nrg['user']['relationship_id'])) {
        $sub_sql .= " `relationship_id`  = '" . $nrg['user']['relationship_id'] . "' ";
    }
    if (!empty($nrg['user']['school'])) {
        if (!empty($sub_sql)) {
            $sub_sql .= " OR `school`  = '" . $nrg['user']['school'] . "' ";
        } else {
            $sub_sql .= " `school`  = '" . $nrg['user']['school'] . "' ";
        }
    }
    if (!empty($nrg['user']['working'])) {
        if (!empty($sub_sql)) {
            $sub_sql .= " OR `working`  = '" . $nrg['user']['working'] . "' ";
        } else {
            $sub_sql .= " `working`  = '" . $nrg['user']['working'] . "' ";
        }
    }
    if (!empty($nrg['user']['birthday']) && $nrg['user']['birthday'] != '0000-00-00') {
        if (!empty($sub_sql)) {
            $sub_sql .= " OR `birthday`  = '" . $nrg['user']['birthday'] . "' ";
        } else {
            $sub_sql .= " `birthday`  = '" . $nrg['user']['birthday'] . "' ";
        }
    }
    if (!empty($nrg['user']['country_id'])) {
        if (!empty($sub_sql)) {
            $sub_sql .= " OR `country_id`  = '" . $nrg['user']['country_id'] . "' ";
        } else {
            $sub_sql .= " `country_id`  = '" . $nrg['user']['country_id'] . "' ";
        }
    }
    if (!empty($nrg['user']['city'])) {
        if (!empty($sub_sql)) {
            $sub_sql .= " OR `city`  = '" . $nrg['user']['city'] . "' ";
        } else {
            $sub_sql .= " `city`  = '" . $nrg['user']['city'] . "' ";
        }
    }
    $sub_sql2 = "";
    ...
    if ($query && mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            if (!empty($nrg['user']['relationship_id']) && !empty($fetched_data['user_data']['relationship_id']) && $nrg['user']['relationship_id'] == $fetched_data['user_data']['relationship_id']) {
                $fetched_data['common_things'] = $fetched_data['common_things'] + 1;
            }
            if (!empty($nrg['user']['school']) && !empty($fetched_data['user_data']['school']) && $nrg['user']['school'] == $fetched_data['user_data']['school']) {
                $fetched_data['common_things'] = $fetched_data['common_things'] + 1;
            }
            if (!empty($nrg['user']['working']) && !empty($fetched_data['user_data']['working']) && $nrg['user']['working'] == $fetched_data['user_data']['working']) {
                $fetched_data['common_things'] = $fetched_data['common_things'] + 1;
            }
            if (!empty($nrg['user']['country_id']) && !empty($fetched_data['user_data']['country_id']) && $nrg['user']['country_id'] == $fetched_data['user_data']['country_id']) {
                $fetched_data['common_things'] = $fetched_data['common_things'] + 1;
            }
            if (!empty($nrg['user']['city']) && !empty($fetched_data['user_data']['city']) && $nrg['user']['city'] == $fetched_data['user_data']['city']) {
                $fetched_data['common_things'] = $fetched_data['common_things'] + 1;
            }
            if (!empty($nrg['user']['birthday']) && !empty($fetched_data['user_data']['birthday']) && $nrg['user']['birthday'] == $fetched_data['user_data']['birthday'] && $nrg['user']['birthday'] != '0000-00-00' && $fetched_data['user_data']['birthday'] != '0000-00-00') {
                $fetched_data['common_things'] = $fetched_data['common_things'] + 1;
            }
            if ($fetched_data['user_data']['share_my_location'] == 1) {
                $data[] = $fetched_data;
...
function GetFundingByUserId($user_id, $limit = 6, $offset = 0)
{
    ...
    if (!empty($funding)) {
        foreach ($funding as $key => $fund) {
            $new_data         = $fund;
            $new_data->image  = NRG_GetMedia($new_data->image);
            ...
            $new_data->user_data = NRG_UserData($fund->user_id);
            $new_data->is_donate = $db->where('funding_id', $fund->id)->where('user_id', $nrg['user']['id'])->getValue(T_FUNDING_RAISE, "COUNT(*)");
...
function GetFundingById($id, $type = 'id')
{
    ...
    if (!empty($funding)) {
        $funding->image        = NRG_GetMedia($funding->image);
        ...
        $funding->user_data = NRG_UserData($funding->user_id);
        $funding->is_donate = 0;
        if ($nrg['loggedin']) {
            $funding->is_donate = $db->where('funding_id', $funding->id)->where('user_id', $nrg['user']['id'])->getValue(T_FUNDING_RAISE, "COUNT(*)");
        }
...
function NRG_GetOfferById($offer_id)
{
    global $nrg, $sqlConnect, $db;
    if (empty($offer_id)) {
        return false;
    ...
    $data     = array();
    $offer_id = NRG_Secure($offer_id);
    $offer    = $db->where('id', $offer_id)->getOne(T_OFFER);
    if (!empty($offer)) {
        $page               = NRG_PageData($offer->page_id);
        $offer->image       = NRG_GetMedia($offer->image);
        $offer->expire_date = date($nrg['config']['date_style'], strtotime($offer->expire_date));
        $offer->offer_text  = $nrg['lang']['free_shipping'];
        $offer->currency    = (!empty($nrg['currencies'][$offer->currency]['symbol'])) ? $nrg['currencies'][$offer->currency]['symbol'] : '$';
        ...
        if ($offer->discount_type == 'buy_get_discount' && !empty($offer->discount_percent) && !empty($offer->buy) && !empty($offer->get_price)) {
            $offer->offer_text = $nrg['lang']['buy'] . ' ' . $offer->buy . ' ' . $nrg['lang']['get'] . ' ' . $offer->get_price . ' / %' . $offer->discount_percent . ' Off';
        }
        if ($offer->discount_type == 'spend_get_off' && !empty($offer->spend) && !empty($offer->amount_off)) {
            $offer->offer_text = $nrg['lang']['spend'] . ' ' . $offer->spend . '' . $offer->currency . ' ' . $nrg['lang']['get'] . ' ' . $offer->amount_off . '' . $offer->currency . ' Off';
        }
        $offer            = (array) $offer;
        $offer['page']    = $page;
        $post             = $db->where('offer_id', $offer_id)->getOne(T_POSTS);
        $offer['post_id'] = $post->id;
        $offer['url']     = NRG_SeoLink('index.php?link1=post&id=' . $post->id);
    }
    return $offer;
}
function NRG_GetAllOffers($filter_data = array())
{
    global $nrg, $sqlConnect;
    $data      = array();
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            $offer  = NRG_GetOfferById($fetched_data['id']);
            $data[] = $offer;
        }
    }
    return $data;
}
function NRG_GetNearbyShops($args = array())
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false || empty($args)) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['page_data'] = NRG_PageData($fetched_data['page_id']);
            $fetched_data['product']   = NRG_GetProduct($fetched_data['product_id']);
            $data[]                    = $fetched_data;
        }
    }
    return $data;
}
function NRG_GetNearbyShopsCount($args = array())
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false || empty($args)) {
        return false;
...
function NRG_GetNearbyBusiness($args = array())
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false || empty($args)) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $fetched_data['page_data']         = NRG_PageData($fetched_data['page_id']);
            $fetched_data['job']               = NRG_GetJobById($fetched_data['job_id']);
            $fetched_data['job']['full_image'] = NRG_GetMedia($fetched_data['job']['image']);
            $data[]                            = $fetched_data;
        }
    }
    return $data;
...
function NRG_GetNearbyBusinessCount($args = array())
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false || empty($args)) {
        return false;
...
function NRG_IfCanGenerateLink($user_id)
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false || empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
    }
    $user_id = NRG_Secure($user_id);
    $time    = 0;
    if ($nrg['config']['expire_user_links'] == 'hour') {
        $time = time() - (60 * 60);
    }
    if ($nrg['config']['expire_user_links'] == 'day') {
        $time = time() - (60 * 60 * 24);
    }
    if ($nrg['config']['expire_user_links'] == 'week') {
        $time = time() - (60 * 60 * 24 * 7);
    }
    if ($nrg['config']['expire_user_links'] == 'month') {
        $time = time() - (60 * 60 * 24 * date("t"));
    }
    if ($nrg['config']['expire_user_links'] == 'year') {
    ...
    if (mysqli_num_rows($query)) {
        $fetched_data = mysqli_fetch_assoc($query);
        if ($nrg['config']['user_links_limit'] > 0) {
            if ($nrg['config']['user_links_limit'] > $fetched_data['count']) {
                return true;
            } else {
                return false;
            }
...
function NRG_GetAvailableLinks($user_id)
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false || empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        $fetched_data = mysqli_fetch_assoc($query);
        if ($nrg['config']['user_links_limit'] > 0) {
            return $nrg['config']['user_links_limit'] - $fetched_data['count'];
        } else {
            return $nrg['lang']['unlimited'];
        }
    }
    return false;
}
function NRG_GetGeneratedLinks($user_id)
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false || empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
...
function NRG_GetUsedLinks($user_id)
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false || empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
...
function NRG_GetMyInvitaionCodes($user_id)
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false || empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            if (!empty($fetched_data['invited_id'])) {
                $user_data                 = NRG_UserData($fetched_data['invited_id']);
...
function NRG_AddInvitedUser($user_id, $code)
{
    global $nrg, $sqlConnect, $db;
...
function NRG_IsUserInvitationExists($code = false)
{
    global $sqlConnect, $nrg;
    if (!$code) {
        return false;
...
function NRG_GetAllInvitaionCodes()
{
    global $nrg, $sqlConnect;
    if ($nrg['loggedin'] == false) {
        return false;
...
function NRG_DeleteUserInvitation($col = '', $val = false)
{
    global $sqlConnect, $nrg;
    if (!$val && !$col) {
        return false;
...
function NRG_notifyUsersLive($post_id)
{
    global $sqlConnect, $nrg;
    if ($nrg['loggedin'] == false || empty($post_id) || !is_numeric($post_id) || $post_id < 1) {
        return false;
    }
    $post_id    = NRG_Secure($post_id);
    $data       = array();
...
function NRG_CheckRazorpayPayment($payment_id, $data)
{
    global $nrg;
    ...
    $key_id     = $nrg['config']['razorpay_key_id'];
    $key_secret = $nrg['config']['razorpay_key_secret'];
...
function NRG_CheckAnonymous($id, $type)
{
    global $sqlConnect, $nrg;
...
function StartCloudRecording($vendor, $region, $bucket, $accessKey, $secretKey, $cname, $uid, $post_id, $token)
{
    ...
    $post_id = NRG_Secure($post_id);
    $ch      = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://api.agora.io/v1/apps/" . $nrg['config']['agora_app_id'] . "/cloud_recording/acquire");
    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        'Authorization: Basic ' . base64_encode($nrg['config']['agora_customer_id'] . ":" . $nrg['config']['agora_customer_certificate']),
    ...
    $ch         = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://api.agora.io/v1/apps/" . $nrg['config']['agora_app_id'] . "/cloud_recording/resourceid/" . $resourceId . "/mode/mix/start");
    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        'Authorization: Basic ' . base64_encode($nrg['config']['agora_customer_id'] . ":" . $nrg['config']['agora_customer_certificate']),
        'Content-Type: application/json;charset=utf-8'
    ));
...
{
    global $sqlConnect, $nrg, $db;
    if (empty($data) || $nrg['config']['agora_live_video'] != 1 || empty($data['resourceId']) || empty($data['sid']) || empty($data['cname']) || empty($data['uid']) || empty($data['post_id'])) {
        return false;
    }
    $post_id = NRG_Secure($data['post_id']);
    $ch      = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://api.agora.io/v1/apps/" . $nrg['config']['agora_app_id'] . "/cloud_recording/resourceid/" . $data['resourceId'] . "/sid/" . $data['sid'] . "/mode/mix/stop");
    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        'Authorization: Basic ' . base64_encode($nrg['config']['agora_customer_id'] . ":" . $nrg['config']['agora_customer_certificate']),
        'Content-Type: application/json;charset=utf-8'
    ));
...
function GetBroadcastChatById($id, $user_id = 0)
{
    ...
    $broadcast = $db->where('id', NRG_Secure($id))->where('user_id', $user_id)->getOne(T_CAST);
    if (!empty($broadcast)) {
        $broadcast->org_image = $broadcast->image;
        $broadcast->image     = NRG_GetMedia($broadcast->image);
        $broadcast->users     = array();
        $users                = $db->where('broadcast_id', $broadcast->id)->get(T_CAST_USERS);
        foreach ($users as $key => $value) {
            $broadcast->users[] = NRG_UserData($value->user_id);
        }
        return $broadcast;
    }
    return false;
...
function GetBroadcastChatByUserId($user_id = 0, $limit = 10, $offset = 0)
{
    ...
    if (!empty($offset) && is_numeric($offset) && $offset > 0) {
        $db->where('time', NRG_Secure($offset), '<');
    }
    $limit     = NRG_Secure($limit);
...
{
    global $nrg, $sqlConnect, $db;
    if ($nrg['loggedin'] == false || $nrg['config']['ffmpeg_system'] != 'on' || empty($data) || empty($data['post_data']) || empty($data['filename'])) {
        return false;
    }
    $ffmpeg_b = $nrg['config']['ffmpeg_binary_file'];
    ...
    $shell                         = shell_exec("$ffmpeg_b -y -i $video_file_full_path -vcodec libx264 -preset " . $nrg['config']['convert_speed'] . " -filter:v scale=426:-2 -crf 26 $video_output_full_path_240 2>&1");
    $data['post_data']['postFile'] = $video_path_240;
    $data['id']                    = NRG_RegisterPost($data['post_data']);
    if (file_exists($video_output_full_path_240)) {
        if ($nrg['config']['amazone_s3'] == 1 || $nrg['config']['wasabi_storage'] == 1 || $nrg['config']['ftp_upload'] == 1 || $nrg['config']['spaces'] == 1 || $nrg['config']['cloud_upload'] == 1 || $nrg['config']['backblaze_storage'] == 1) {
            $upload_s3 = NRG_UploadToS3($video_path_240);
        }
        $processing = 0;
        ...
        $notification_data_array = array(
            'recipient_id' => $nrg['user']['user_id'],
            'type' => 'admin_notification',
            'time' => time(),
            'url' => 'index.php?link1=post&id=' . $data['id'],
            'text' => $nrg['lang']['video_ready_to_view'],
            'type2' => 'ffmpeg'
        );
        $db->insert(T_NOTIFICATION, $notification_data_array);
    }
    if (empty($data['video_thumb'])) {
        ...
        $hash       = sha1(time() + time() - rand(9999, 9999)) . NRG_GenerateKey();
        ...
        if ($nrg['config']['amazone_s3'] == 1 || $nrg['config']['wasabi_storage'] == 1 || $nrg['config']['ftp_upload'] == 1 || $nrg['config']['spaces'] == 1 || $nrg['config']['cloud_upload'] == 1 || $nrg['config']['backblaze_storage'] == 1) {
            $upload_s3 = NRG_UploadToS3($file_thumb);
        }
        $db->where('id', $data['id'])->update(T_POSTS, array(
    ...
    if ($resolution >= 640 || $resolution == 0) {
        $shell = shell_exec("$ffmpeg_b -y -i $video_file_full_path -vcodec libx264 -preset " . $nrg['config']['convert_speed'] . " -filter:v scale=640:-2 -crf 26 $video_output_full_path_360 2>&1");
        if (file_exists($video_output_full_path_360)) {
            if ($nrg['config']['amazone_s3'] == 1 || $nrg['config']['wasabi_storage'] == 1 || $nrg['config']['ftp_upload'] == 1 || $nrg['config']['spaces'] == 1 || $nrg['config']['cloud_upload'] == 1 || $nrg['config']['backblaze_storage'] == 1) {
                $upload_s3 = NRG_UploadToS3($video_path_360);
            }
            $db->where('id', $data['id'])->update(T_POSTS, array(
                '360p' => 1
            ));
    ...
    if ($resolution >= 854 || $resolution == 0) {
        $shell = shell_exec("$ffmpeg_b -y -i $video_file_full_path -vcodec libx264 -preset " . $nrg['config']['convert_speed'] . " -filter:v scale=854:-2 -crf 26 $video_output_full_path_480 2>&1");
        if (file_exists($video_output_full_path_480)) {
            if ($nrg['config']['amazone_s3'] == 1 || $nrg['config']['wasabi_storage'] == 1 || $nrg['config']['ftp_upload'] == 1 || $nrg['config']['spaces'] == 1 || $nrg['config']['cloud_upload'] == 1 || $nrg['config']['backblaze_storage'] == 1) {
                $upload_s3 = NRG_UploadToS3($video_path_480);
            }
            $db->where('id', $data['id'])->update(T_POSTS, array(
                '480p' => 1
            ));
    ...
    if ($resolution >= 1280 || $resolution == 0) {
        $shell = shell_exec("$ffmpeg_b -y -i $video_file_full_path -vcodec libx264 -preset " . $nrg['config']['convert_speed'] . " -filter:v scale=1280:-2 -crf 26 $video_output_full_path_720 2>&1");
        if (file_exists($video_output_full_path_720)) {
            if ($nrg['config']['amazone_s3'] == 1 || $nrg['config']['wasabi_storage'] == 1 || $nrg['config']['ftp_upload'] == 1 || $nrg['config']['spaces'] == 1 || $nrg['config']['cloud_upload'] == 1 || $nrg['config']['backblaze_storage'] == 1) {
                $upload_s3 = NRG_UploadToS3($video_path_720);
            }
            $db->where('id', $data['id'])->update(T_POSTS, array(
                '720p' => 1
            ));
    ...
    if ($resolution >= 1920 || $resolution == 0) {
        $shell = shell_exec("$ffmpeg_b -y -i $video_file_full_path -vcodec libx264 -preset " . $nrg['config']['convert_speed'] . " -filter:v scale=1920:-2 -crf 26 $video_output_full_path_1080 2>&1");
        if (file_exists($video_output_full_path_1080)) {
            if ($nrg['config']['amazone_s3'] == 1 || $nrg['config']['wasabi_storage'] == 1 || $nrg['config']['ftp_upload'] == 1 || $nrg['config']['spaces'] == 1 || $nrg['config']['cloud_upload'] == 1 || $nrg['config']['backblaze_storage'] == 1) {
                $upload_s3 = NRG_UploadToS3($video_path_1080);
            }
            $db->where('id', $data['id'])->update(T_POSTS, array(
                '1080p' => 1
            ));
    ...
    if ($resolution >= 2048 || $resolution == 0) {
        $shell = shell_exec("$ffmpeg_b -y -i $video_file_full_path -vcodec libx264 -preset " . $nrg['config']['convert_speed'] . " -filter:v scale=2048:-2 -crf 26 $video_output_full_path_2048 2>&1");
        if (file_exists($video_output_full_path_2048)) {
            if ($nrg['config']['amazone_s3'] == 1 || $nrg['config']['wasabi_storage'] == 1 || $nrg['config']['ftp_upload'] == 1 || $nrg['config']['spaces'] == 1 || $nrg['config']['cloud_upload'] == 1 || $nrg['config']['backblaze_storage'] == 1) {
                $upload_s3 = NRG_UploadToS3($video_path_2048);
            }
            $db->where('id', $data['id'])->update(T_POSTS, array(
                '2048p' => 1
            ));
    ...
    if ($resolution >= 3840 || $resolution == 0) {
        $shell = shell_exec("$ffmpeg_b -y -i $video_file_full_path -vcodec libx264 -preset " . $nrg['config']['convert_speed'] . " -filter:v scale=3840:-2 -crf 26 $video_output_full_path_4096 2>&1");
        if (file_exists($video_output_full_path_4096)) {
            if ($nrg['config']['amazone_s3'] == 1 || $nrg['config']['wasabi_storage'] == 1 || $nrg['config']['ftp_upload'] == 1 || $nrg['config']['spaces'] == 1 || $nrg['config']['cloud_upload'] == 1 || $nrg['config']['backblaze_storage'] == 1) {
                $upload_s3 = NRG_UploadToS3($video_path_4096);
            }
            $db->where('id', $data['id'])->update(T_POSTS, array(
                '4096p' => 1
            ));
...
function GetReview($id)
{
    ...
    $review      = $db->where('id', NRG_Secure($id))->getOne(T_PRODUCT_REVIEW);
    $images      = $db->where('review_id', $review->id)->get(T_ALBUMS_MEDIA);
    $images_data = array();
    foreach ($images as $key => $value) {
        $new              = array();
        $new['org_image'] = $value->image;
        $new['image']     = NRG_GetMedia($value->image);
        $review->images[] = $new;
    }
    $review->user_data = NRG_UserData($review->user_id);
    return $review;
}
function EnableForMode($value, $class = false, $title = false)
{
    ...
    if ((!empty($nrg['website_modes_off']) && !empty($nrg['website_modes_off'][$nrg['config']['website_mode']]) && in_array($value, $nrg['website_modes_off'][$nrg['config']['website_mode']]))) {
        if ($class == true) {
            return 'ch_light_color';
        }
        if ($title == true) {
            return 'title="Your website in ' . ucfirst($nrg['config']['website_mode']) . ' Mode You will not be able to enable this feature"';
        }
        return 'disabled';
    }
    return true;
...
function TextForMode($value)
{
    ...
    if (!empty($nrg['website_modes_text'][$nrg['config']['website_mode']]) && !empty($nrg['website_modes_text'][$nrg['config']['website_mode']][$value])) {
        if (!empty($nrg['notification']) && !empty($nrg['notification']['post_id'])) {
            $is_live = $db->where('post_id', $nrg['notification']['post_id'])->where("live_time", 0, '!=')->getValue(T_POSTS, 'COUNT(*)');
            if ($is_live > 0) {
                return $nrg['lang'][$value];
            }
        }
        return $nrg['lang'][$nrg['website_modes_text'][$nrg['config']['website_mode']][$value]];
    }
    return $nrg['lang'][$value];
}
...
{
    global $nrg, $sqlConnect, $db;
    if ($nrg['config']['website_mode'] != 'patreon') {
        return '';
    }
    if ($nrg['loggedin'] == false) {
        return 'filter: blur(3px);';
    }
    if ($nrg['story']['user_id'] == $nrg['user']['id'] || NRG_IsAdmin()) {
        return '';
    }
    $is_subscribed = $db->where('user_id', $nrg['story']['user_id'])->where('subscriber_id', $nrg['user']['id'])->getValue(T_PATREON_SUBSCRIBERS, 'COUNT(*)');
    if ($is_subscribed > 0) {
        return '';
    }
    return 'filter: blur(3px);';
...
function PatreonSubscribed($user_id)
{
    ...
    if ($user_id == $nrg['user']['id'] || NRG_IsAdmin()) {
        return true;
    }
    $is_subscribed = $db->where('user_id', $user_id)->where('subscriber_id', $nrg['user']['id'])->getValue(T_PATREON_SUBSCRIBERS, 'COUNT(*)');
    if ($is_subscribed > 0) {
        return true;
    }
    return false;
...
function PatreonPostSubscribed($user_id, $post_id)
{
    ...
    $post_patron = $db->where('id', NRG_Secure($post_id))->where('user_id', NRG_Secure($user_id))->where('postPrivacy', '5')->getValue(T_POSTS, 'COUNT(*)');
    if ($post_patron > 0) {
        $is_subscribed = $db->where('user_id', $user_id)->where('subscriber_id', $nrg['user']['id'])->getValue(T_PATREON_SUBSCRIBERS, 'COUNT(*)');
        if ($is_subscribed > 0) {
            return true;
...
function NRG_GetSearchServices($query = '')
{
    global $nrg, $sqlConnect, $db;
    $data = array();
    if (empty($query)) {
    ...
    if (!empty($services)) {
        foreach ($services as $key => $value) {
            $data[$value->user_id] = NRG_UserData($value->user_id);
        }
        return $data;
    }
    return array();
}
function NRG_UserSugServices($limit = 20)
{
    global $nrg, $sqlConnect, $db;
    if (!is_numeric($limit)) {
        return false;
...
function NRG_GetOpenToWorkPosts($limit = 10, $offset = 0)
{
    global $nrg, $sqlConnect, $db;
...
function AddNewRef($ref_id, $user_id, $amount)
{
    ...
    if ($nrg['config']['affiliate_level'] < 2) {
        return false;
    }
    $user_id  = NRG_Secure($user_id);
    $ref_id   = NRG_Secure($ref_id);
    $amount   = NRG_Secure($amount);
    $pre_user = 0;
    $parents  = array();
    for ($i = 0; $i < $nrg['config']['affiliate_level']; $i++) {
