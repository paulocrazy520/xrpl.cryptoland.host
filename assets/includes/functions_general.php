...
function NRG_LoadPage($page_url = '')
{
    global $nrg, $db;
    $create_file = false;
    if ($page_url == 'sidebar/content' && $nrg['loggedin'] == true && $nrg['config']['cache_sidebar'] == 1) {
        $file_path = './cache/sidebar-' . $nrg['user']['user_id'] . '.tpl';
    ...
    $page         = './themes/' . $nrg['config']['theme'] . '/layout/' . $page_url . '.phtml';
    $page_content = '';
    ob_start();
    require($page);
    $page_content = ob_get_contents();
    ob_end_clean();
    if ($create_file == true && $nrg['config']['cache_sidebar'] == 1) {
        $create_sidebar_file = file_put_contents($file_path, $page_content);
        setcookie("last_sidebar_update", time(), time() + (10 * 365 * 24 * 60 * 60));
    }
    return $page_content;
}
function NRG_CleanCache($user_id = '', $where = 'sidebar')
{
    global $nrg;
    if ($nrg['config']['cache_sidebar'] == 0 || $nrg['loggedin'] == false) {
        return false;
    }
    $file_path = './cache/sidebar-' . $nrg['user']['user_id'] . '.tpl';
    if (file_exists($file_path)) {
...
function NRG_CustomCode($a = false, $code = array())
{
    global $nrg;
    $theme       = $nrg['config']['theme'];
    $data        = array();
    $result      = false;
    $custom_code = array(
...
function NRG_LoadAdminPage($page_url = '')
{
    global $nrg, $db;
...
function NRG_LoadAdminLinkSettings($link = '')
{
    global $site_url;
    return $site_url . '/admin-cp/' . $link;
}
function NRG_LoadAdminLink($link = '')
{
    global $site_url;
    return $site_url . '/admin-panel/' . $link;
}
function NRG_SizeUnits($bytes = 0)
{
    if ($bytes >= 1073741824) {
...
function NRG_MultipleArrayFiles($file_post)
{
    if (!is_array($file_post)) {
        return array();
    }
    $nrg_file_array = array();
    $nrg_file_count = count($file_post['name']);
    $nrg_file_keys  = array_keys($file_post);
    for ($i = 0; $i < $nrg_file_count; $i++) {
        foreach ($nrg_file_keys as $key) {
            $nrg_file_array[$i][$key] = $file_post[$key][$i];
        }
    }
    return $nrg_file_array;
}
function NRG_IsValidMimeType($mimeTypes = array())
{
...
function NRG_SeoLink($query = '')
{
    global $nrg, $config;
    if ($nrg['config']['seoLink'] == 1) {
...
function NRG_IsLogged()
{
    if (isset($_SESSION['user_id']) && !empty($_SESSION['user_id'])) {
        $id = NRG_GetUserFromSessionID($_SESSION['user_id']);
        if (is_numeric($id) && !empty($id)) {
            return true;
        }
    } else if (!empty($_COOKIE['user_id']) && !empty($_COOKIE['user_id'])) {
        $id = NRG_GetUserFromSessionID($_COOKIE['user_id']);
        if (is_numeric($id) && !empty($id)) {
            return true;
        }
    } else {
...
function NRG_Redirect($url)
{
    return header("Location: {$url}");
}
function NRG_Link($string)
{
    global $site_url;
...
function NRG_BbcodeSecure($string)
{
    global $sqlConnect;
    $string = trim($string);
    $string = mysqli_real_escape_string($sqlConnect, $string);
...
function NRG_Decode($string)
{
    return htmlspecialchars_decode($string);
}
function NRG_GenerateKey($minlength = 20, $maxlength = 20, $uselower = true, $useupper = true, $usenumbers = true, $usespecial = false)
{
    $charset = '';
    if ($uselower) {
...
$can = 0;
function NRG_CropAvatarImage($file = '', $data = array())
{
    global $nrg;
    if (empty($file)) {
        return false;
    ...
    if (!file_exists($file)) {
        $get_media = file_put_contents($file, file_get_contents(NRG_GetMedia($file)));
    }
    if (!file_exists($file)) {
        return false;
    }
    ...
    NRG_Resize_Crop_Image($nrg['profile_picture_width_crop'], $nrg['profile_picture_height_crop'], $file, $file, 80);
    $s3 = NRG_UploadToS3($file);
    return true;
}
function NRG_Resize_Crop_Image($max_width, $max_height, $source_file, $dst_dir, $quality = 80)
{
...
function NRG_Time_Elapsed_String_word($ptime)
{
    global $nrg;
    $etime = time() - $ptime;
    if ($etime < 1) {
    ...
    $a        = array(
        365 * 24 * 60 * 60 => $nrg['lang']['year'],
        30 * 24 * 60 * 60 => $nrg['lang']['month'],
        24 * 60 * 60 => $nrg['lang']['day'],
        60 * 60 => $nrg['lang']['hour'],
        60 => $nrg['lang']['minute'],
        1 => $nrg['lang']['second']
    );
    $a_plural = array(
        $nrg['lang']['year'] => $nrg['lang']['years'],
        $nrg['lang']['month'] => $nrg['lang']['months'],
        $nrg['lang']['day'] => $nrg['lang']['days'],
        $nrg['lang']['hour'] => $nrg['lang']['hours'],
        $nrg['lang']['minute'] => $nrg['lang']['minutes'],
        $nrg['lang']['second'] => $nrg['lang']['seconds']
    );
    foreach ($a as $secs => $str) {
        ...
        if ($d >= 1) {
            ...
            if ($nrg['language_type'] == 'rtl') {
                $time_ago = $nrg['lang']['time_ago'] . ' ' . $r . ' ' . ($r > 1 ? $a_plural[$str] : $str);
            } else {
                $time_ago = $r . ' ' . ($r > 1 ? $a_plural[$str] : $str) . ' ' . $nrg['lang']['time_ago'];
            }
            return $time_ago;
        }
    }
}
function NRG_Time_Elapsed_String($ptime)
{
    global $nrg;
    $etime = (time()) - $ptime;
    if ($etime < 1) {
    ...
    $years   = $days / 365;
    if ($seconds < 45) {
        return substitute($nrg['lang']['now'], '');
    } elseif ($seconds < 90) {
        return substitute($nrg['lang']['_time_m'], 1);
    } elseif ($minutes < 45) {
        return substitute($nrg['lang']['_time_m'], round($minutes));
    } elseif ($minutes < 90) {
        return substitute($nrg['lang']['_time_h'], 1);
    } elseif ($hours < 24) {
        return substitute($nrg['lang']['_time_hrs'], round($hours));
    } elseif ($hours < 42) {
        return substitute($nrg['lang']['_time_d'], 1);
    } elseif ($days < 7) {
        return substitute($nrg['lang']['_time_d'], round($days));
    } elseif ($weeks < 2) {
        return substitute($nrg['lang']['_time_w'], 1);
    } elseif ($weeks < 52) {
        return substitute($nrg['lang']['_time_w'], round($weeks));
    } elseif ($years < 1.5) {
        return substitute($nrg['lang']['_time_y'], 1);
    } else {
        return substitute($nrg['lang']['_time_yrs'], round($years));
    }
    // $a        = array(
    //     365 * 24 * 60 * 60 => $nrg['lang']['year'],
    //     30 * 24 * 60 * 60 => $nrg['lang']['month'],
    ...
    foreach ($dir_array as $key => $filename) {
        if ($filename != ".." && $filename != "." && $filename != ".htaccess") {
            if (is_dir($dir . "/" . $filename)) {
                $new_foldersize = NRG_FolderSize($dir . "/" . $filename);
...
function NRG_GetThemes()
{
    global $nrg;
    $themes = glob('themes/*', GLOB_ONLYDIR);
    return $themes;
}
function NRG_ReturnBytes($val)
{
    $val  = trim($val);
...
function NRG_MaxFileUpload()
{
    //select maximum upload size
    $max_upload   = NRG_ReturnBytes(ini_get('upload_max_filesize'));
    //select post limit
    $max_post     = NRG_ReturnBytes(ini_get('post_max_size'));
    //select memory limit
    $memory_limit = NRG_ReturnBytes(ini_get('memory_limit'));
    // return the smallest of them, this defines the real limit
    return min($max_upload, $max_post, $memory_limit);
}
function NRG_CompressImage($source_url, $destination_url, $quality)
{
...
function NRG_Backup($sql_db_host, $sql_db_user, $sql_db_pass, $sql_db_name, $tables = false, $backup_name = false)
{
...
function NRG_CheckUserSessionID($user_id = 0, $session_id = '', $platform = 'web')
{
...
function NRG_ValidateAccessToken($access_token = '')
{
    global $nrg, $sqlConnect;
    if (empty($access_token)) {
        return false;
    }
    $access_token = NRG_Secure($access_token);
...
function NRG_UploadToS3($filename, $config = array())
{
    global $nrg;
    if ($nrg['config']['amazone_s3'] == 0 && $nrg['config']['ftp_upload'] == 0 && $nrg['config']['spaces'] == 0 && $nrg['config']['cloud_upload'] == 0 && $nrg['config']['wasabi_storage'] == 0 && $nrg['config']['backblaze_storage'] == 0) {
        return false;
    }
    if (empty($filename)) {
        return false;
    ...
    if ($nrg['config']['ftp_upload'] == 1) {
        include_once('assets/libraries/ftp/vendor/autoload.php');
        $ftp = new \FtpClient\FtpClient();
        $ftp->connect($nrg['config']['ftp_host'], false, $nrg['config']['ftp_port']);
        $login = $ftp->login($nrg['config']['ftp_username'], $nrg['config']['ftp_password']);
        if ($login) {
            if (!empty($nrg['config']['ftp_path'])) {
                if ($nrg['config']['ftp_path'] != "./") {
                    $ftp->chdir($nrg['config']['ftp_path']);
                }
            }
    ...
    } else if ($nrg['config']['amazone_s3'] == 1) {
        ...
        $s3 = new S3Client(array(
            'version' => 'latest',
            'region' => $nrg['config']['region'],
            'credentials' => array(
                'key' => $nrg['config']['amazone_s3_key'],
                'secret' => $nrg['config']['amazone_s3_s_key']
            )
        ));
        $s3->putObject(array(
            'Bucket' => $nrg['config']['bucket_name'],
            'Key' => $filename,
        ...
        if (empty($config['delete'])) {
            if ($s3->doesObjectExist($nrg['config']['bucket_name'], $filename)) {
                if (empty($config['amazon'])) {
                    @unlink($filename);
                }
                return true;
    ...
    } else if ($nrg['config']['wasabi_storage'] == 1) {
        ...
        $s3 = new S3Client(array(
            'version' => 'latest',
            'endpoint' => 'https://s3.' . $nrg['config']['wasabi_bucket_region'] . '.wasabisys.com',
            'region' => $nrg['config']['wasabi_bucket_region'],
            'credentials' => array(
                'key' => $nrg['config']['wasabi_access_key'],
                'secret' => $nrg['config']['wasabi_secret_key']
            )
        ));
        $s3->putObject(array(
            'Bucket' => $nrg['config']['wasabi_bucket_name'],
            'Key' => $filename,
            'Body' => fopen($filename, 'r+'),
            'ACL' => 'public-read',
            'CacheControl' => 'max-age=3153600'
        ...
        if (empty($config['delete'])) {
            if ($s3->doesObjectExist($nrg['config']['wasabi_bucket_name'], $filename)) {
        ...
        include_once('assets/libraries/s3-lib/vendor/autoload.php');
        $key        = $nrg['config']['spaces_key'];
        $secret     = $nrg['config']['spaces_secret'];
        $spaceName = $nrg['config']['space_name'];
        $region     = $nrg['config']['space_region'];
        ...
        if (empty($config['delete'])) {
            if ($s3->doesObjectExist($nrg['config']['space_name'], $filename)) {
                if (empty($config['amazon'])) {
                    @unlink($filename);
                }
                return true;
    ...
    } elseif ($nrg['config']['backblaze_storage'] == 1) {
        ...
        return false;
    } elseif ($nrg['config']['cloud_upload'] == 1) {
        require_once 'assets/libraries/google-lib/vendor/autoload.php';
        try {
            $storage       = new StorageClient(array(
                'keyFilePath' => $nrg['config']['cloud_file_path']
            ));
            // set which bucket to work in
            $bucket        = $storage->bucket($nrg['config']['cloud_bucket_name']);
...
function NRG_DeleteFromToS3($filename, $config = array())
{
    global $nrg;
    if ($nrg['config']['amazone_s3'] == 0 && $nrg['config']['ftp_upload'] == 0 && $nrg['config']['spaces'] == 0 && $nrg['config']['cloud_upload'] == 0 && $nrg['config']['amazone_s3_2'] == 0 && $nrg['config']['wasabi_storage'] == 0 && $nrg['config']['backblaze_storage'] == 0) {
        return false;
    }
    if (empty($filename)) {
        return false;
    ...
    if ($nrg['config']['amazone_s3_2'] == 1) {
        include_once('assets/libraries/s3-lib/vendor/autoload.php');
        if (empty($nrg['config']['amazone_s3_key_2']) || empty($nrg['config']['amazone_s3_s_key_2']) || empty($nrg['config']['region_2']) || empty($nrg['config']['bucket_name_2'])) {
            return false;
        }
        $s3 = new S3Client(array(
            'version' => 'latest',
            'region' => $nrg['config']['region_2'],
            'credentials' => array(
                'key' => $nrg['config']['amazone_s3_key_2'],
                'secret' => $nrg['config']['amazone_s3_s_key_2']
            )
        ));
        $s3->deleteObject(array(
            'Bucket' => $nrg['config']['bucket_name_2'],
            'Key' => $filename
        ));
        if (!$s3->doesObjectExist($nrg['config']['bucket_name_2'], $filename)) {
            return true;
        }
    }
}
...
function NRG_CanBlog()
{
    global $nrg;
    if ($nrg['config']['blogs'] == 1) {
        if ($nrg['config']['can_use_blog']) {
            return true;
        }
        return false;
    }
...
function NRG_GetIcon($icon)
{
    global $nrg;
    return $nrg['config']['theme_url'] . '/icons/png/' . $icon . '.png';
}
function NRG_IsFileAllowed($file_name, $fileType = '')
{
    global $nrg;
    $new_string        = pathinfo($file_name, PATHINFO_FILENAME) . '.' . strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
    if ($nrg['config']['audio_upload'] == 0) {
        $nrg['config']['allowedExtenstion'] = str_replace(array(',mp3', ',wav'), '', $nrg['config']['allowedExtenstion']);
    }
    if ($nrg['config']['video_upload'] == 0) {
        $nrg['config']['allowedExtenstion'] = str_replace(array(',mp4', ',flv', ',mov', ',avi', ',webm', ',mpeg'), '', $nrg['config']['allowedExtenstion']);
    }
    $extension_allowed = explode(',', $nrg['config']['allowedExtenstion']);
...
function NRG_IsVideoNotAllowedMime($file_type)
{
    global $nrg;
    $mime_types = explode(',', $nrg['config']['ffmpeg_mime_types']);
    if (!in_array($file_type, $mime_types)) {
        return true;
    }
    return false;
}
function NRG_IsFfmpegFileAllowed($file_name)
{
    global $nrg;
    $new_string        = pathinfo($file_name, PATHINFO_FILENAME) . '.' . strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
    $extension_allowed = explode(',', $nrg['config']['allowedffmpegExtenstion']);
    $file_extension    = pathinfo($new_string, PATHINFO_EXTENSION);
    if (in_array($file_extension, $extension_allowed)) {
        return true;
    }
...
function NRG_ShortText($text = "", $len = 100)
{
...
function NRG_DelexpiredEnvents()
{
    global $nrg, $sqlConnect;
    $t_events     = T_EVENTS;
...
function NRG_RunInBackground($data = array())
{
...
function watermark_image($target)
{
    ...
    if ($nrg['config']['watermark'] != 1) {
        return false;
    }
    try {
        $image = new \claviska\SimpleImage();
        $image->fromFile($target)->autoOrient()->overlay("./themes/{$nrg['config']['theme']}/img/icon.png", 'top left', 1, 30, 30)->toFile($target, 'image/jpeg');
        return true;
    } catch (Exception $err) {
