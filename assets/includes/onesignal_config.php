<?php
function NRG_SendPushNotification($data = array(), $push_type = 'chat') {
    ...
    if ($push_type == 'android_messenger') {
        $app_id  = $nrg['config']['android_m_push_id'];
        $app_key = $nrg['config']['android_m_push_key'];
    } else if ($push_type == 'ios_messenger') {
        $app_id  = $nrg['config']['ios_m_push_id'];
        $app_key = $nrg['config']['ios_m_push_key'];
    } else if ($push_type == 'android_native') {
        $app_id  = $nrg['config']['android_n_push_id'];
        $app_key = $nrg['config']['android_n_push_key'];
    } else if ($push_type == 'ios_native') {
        $app_id  = $nrg['config']['ios_n_push_id'];
        $app_key = $nrg['config']['ios_n_push_key'];
    } else if ($push_type == 'web') {
        $app_id  = $nrg['config']['web_push_id'];
        $app_key = $nrg['config']['web_push_key'];
    }
    $data['notification']['notification_content'] = NRG_EmoPhone($data['notification']['notification_content']);
    $data['notification']['notification_content'] = NRG_EditMarkup($data['notification']['notification_content']);
